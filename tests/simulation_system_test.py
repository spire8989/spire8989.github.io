...
def run():
    ...
    try:
        ...
        check(
            "(() => { const choices=id=>ENCOUNTER_DEFINITIONS[id].stages.start.choices; const choose=(id, list=choices(id))=>SimulationStrategies.cautious.chooseEncounter(list,{encounter:ENCOUNTER_DEFINITIONS[id],stage:ENCOUNTER_DEFINITIONS[id].stages.start,stageId:'start',campaignGoal:null,random:GameRandom.create('cautious-choice-test').random}); const camp=choices('abandoned_camp'); const purse=choices('lost_purse'); const cart=choices('abandoned_cart'); const bundle=choices('discarded_bundle'); const recipe={id:'recipe_reward_fixture',outcomes:[{type:'learnRecipe',recipeId:'not-hard-coded-recipe'}]}; const combat={id:'combat_fixture',outcomes:[{type:'startCombat',combatId:'wild_boar'}]}; const health={id:'health_fixture',outcomes:[{type:'modifyResource',resource:'health',amount:-1}]}; const randomRisk={id:'random_risk_fixture',outcomes:[{type:'randomChance',chance:0.5,effects:[{type:'gainUnsecuredItem',itemId:'old_coin',quantity:1}],elseEffects:[{type:'modifyResource',resource:'health',amount:-1}]}]}; const unknown={id:'unknown_fixture',outcomes:[{type:'futureEffect'}]}; const neutralA={id:'neutral_a',label:'First',outcomes:[]}; const neutralB={id:'neutral_b',label:'Second',outcomes:[]}; const safe=(choice)=>SimulationChoiceSafety.analyze(choice); return safe(camp.find(choice=>choice.id==='search_camp')).classification==='clearly-safe'&&choose('abandoned_camp',camp).id==='search_camp'&&safe(purse.find(choice=>choice.id==='pick_up')).classification==='clearly-safe'&&choose('lost_purse',purse).id==='pick_up'&&safe(cart.find(choice=>choice.id==='search_cart')).classification==='clearly-safe'&&choose('abandoned_cart',cart).id==='search_cart'&&safe(bundle.find(choice=>choice.id==='open_bundle')).classification==='clearly-safe'&&choose('discarded_bundle',bundle).id==='open_bundle'&&safe(recipe).classification==='clearly-safe'&&safe(recipe).positiveUtility>0&&safe(combat).classification==='dangerous'&&safe(health).classification!=='clearly-safe'&&safe(randomRisk).classification!=='clearly-safe'&&safe(unknown).classification==='unknown'&&SimulationStrategies.cautious.chooseEncounter([unknown,neutralA],{encounter:{id:'fixture'},stage:{},stageId:'start',random:GameRandom.create('unknown-choice').random})===neutralA&&SimulationStrategies.cautious.chooseEncounter([neutralA,neutralB],{encounter:{id:'fixture'},stage:{},stageId:'start',random:GameRandom.create('neutral-choice').random})===neutralA; })()",
            "Cautious did not classify recursive authored outcomes or prefer safe positive choices",
        )
        check(
            "(() => { const run=SimulationRunner.run({seed:'choice-safety-telemetry',strategy:'cautious',provisions:30,turnaroundPolicy:{type:'fixedDistance',distance:60}}); const selected=run.decisions.filter(decision=>decision.type==='encounter-choice'||decision.type==='camp-event-choice'); const summary=SimulationTelemetry.aggregate([run]); return selected.length>0&&selected.every(decision=>typeof decision.encounterId==='string'&&typeof decision.choiceId==='string'&&typeof decision.safetyClassification==='string'&&Number.isFinite(decision.positiveUtility)&&Number.isFinite(decision.riskScore)&&typeof decision.reason==='string')&&run.choiceSafety?.availableByClassification?.['clearly-safe']>=run.choiceSafety?.selectedByClassification?.['clearly-safe']&&summary.choiceSafety?.selectedByClassification&&SimulationTelemetry.toCsv({results:[run]}).includes('safePositiveChoicesSelected'); })()",
            "Cautious choice safety telemetry was not recorded in runs and batch summaries",
        )
        check(
            "(() => { const make=(health,provisions,packedItems=['bandages'])=>{const p=SaveSystem.createDefaultPlayerState(); p.selectedCompanions=[]; p.selectedCompanion=null; p.ownedItems.bandages=packedItems.includes('bandages')?1:0; const e=ExpeditionRules.createExpedition(p,{companions:[],provisions,health,packedItems,random:()=>0}); return {p,e};}; const fight=ENCOUNTER_DEFINITIONS.wild_boar.stages.start.choices.find(choice=>choice.id==='fight'); const avoid=ENCOUNTER_DEFINITIONS.wild_boar.stages.start.choices.find(choice=>choice.id==='avoid'); const context=(state,direction='outbound',distance=4,target=50)=>({expedition:{...state.e,direction,distance,unsecuredLoot:state.e.unsecuredLoot??[]},player:state.p,encounter:ENCOUNTER_DEFINITIONS.wild_boar,stage:ENCOUNTER_DEFINITIONS.wild_boar.stages.start,stageId:'start',turnaroundPolicy:{type:'fixedDistance',distance:target}}); const healthy=make(40,20); const critical=make(10,20,[]); const low=make(40,5,[]); const plenty=make(40,20,[]); const lowContext=context(low); const plentyContext=context(plenty); const returning=make(40,20); returning.e.direction='returning'; returning.e.distance=20; returning.e.unsecuredLoot=[{itemId:'gilded_brooch',quantity:1}]; const early=make(40,20,[]); const healthyContext=context(healthy); const criticalContext=context(critical); const returningContext=context(returning,'returning',20,50); const earlyContext=context(early,'outbound',4,50); const lowCost={id:'costly',outcomes:[{type:'modifyResource',resource:'provisions',amount:-3}]}; const harmful={id:'harmful',outcomes:[{type:'randomChance',chance:0.5,effects:[{type:'modifyResource',resource:'health',amount:-1}],elseEffects:[]}]}; const safeLoot={id:'safe_loot',outcomes:[{type:'gainUnsecuredItem',itemId:'old_coin',quantity:1}]}; const healthyFight=cautiousChoiceScore(fight,healthyContext); const healthyAvoid=cautiousChoiceScore(avoid,healthyContext); const criticalFight=cautiousChoiceScore(fight,criticalContext); const criticalAvoid=cautiousChoiceScore(avoid,criticalContext); const returningFight=cautiousChoiceScore(fight,returningContext); const returningAvoid=cautiousChoiceScore(avoid,returningContext); const earlyFight=cautiousChoiceScore(fight,earlyContext); const lowCostScore=cautiousChoiceScore(lowCost,lowContext); const plentyCostScore=cautiousChoiceScore(lowCost,plentyContext); return healthyFight>healthyAvoid&&SimulationStrategies.cautious.chooseEncounter([fight,avoid],healthyContext)===fight&&criticalFight<criticalAvoid&&SimulationStrategies.cautious.chooseEncounter([fight,avoid],criticalContext)===avoid&&lowCostScore<plentyCostScore&&returningFight<returningAvoid&&earlyFight>-1000&&healthyFight>-1000&&SimulationStrategies.cautious.chooseEncounter([harmful,safeLoot],healthyContext)===safeLoot; })()",
            "Cautious did not combine context with safety for combat, provisions, cargo, and safe-positive choices",
        )
        check(
            "(() => { const batch=CampaignSimulationRunner.runBatch({scenarios:[{id:'cautious-batch-sanity',strategy:'cautious',betweenExpeditionPolicy:'conservative-sustainer',turnaroundDistance:75,startingState:{currentGold:100,provisions:30}}],campaignsPerScenario:12,expeditionsPerCampaign:5}); const s=batch.summary; const safePositive=batch.results.reduce((total,campaign)=>total+campaign.expeditions.reduce((count,expedition)=>count+(expedition.expeditionTelemetry?.safePositiveChoicesSelected||0),0),0); return s.campaignCompletionRate>0.5&&s.resourceExhaustionRate<0.5&&s.deathRate<0.5&&s.averageExpeditionsSurvived>=4&&s.averageCombats>0.5&&safePositive>0; })()",
            "Cautious still starved instead of making contextual progress in a small campaign batch",
        )
        ...
        check(
            "(() => { const base=SaveSystem.createDefaultPlayerState(); const solo={...base,selectedCompanions:[],selectedCompanion:null,materials:{...base.materials,raw_meat:30}}; const soloBag=ExpeditionRules.createExpedition(solo,{companions:[],provisions:5,materialBagContents:{raw_meat:30},random:()=>0}); const llamrei={...base,selectedCompanions:['llamrei'],selectedCompanion:'llamrei',materials:{...base.materials,raw_meat:30}}; const horseBag=ExpeditionRules.createExpedition(llamrei,{companions:['llamrei'],provisions:5,materialBagContents:{raw_meat:30},random:()=>0}); const simulated=SimulationRunner.run({seed:'llamrei-material-capacity',strategy:'normal',companions:['llamrei'],provisions:10,startingState:{materials:{raw_meat:30}},materialBagContents:{raw_meat:30},turnaroundPolicy:{type:'fixedDistance',distance:5}}); const generic=MaterialRules.effectiveCapacity({playerCharacter:{materialBagCapacity:15},selectedCompanions:[{materialBagCapacityBonus:7}],companionDefinitions:{}}); return PLAYER_CHARACTER_DEFINITION.materialBagCapacity===15&&MaterialRules.capacity()===15&&soloBag.materialBag.capacity===15&&soloBag.materialBag.secured.raw_meat===15&&soloBag.materialBag.rejected.raw_meat===15&&horseBag.materialBag.capacity===25&&horseBag.materialBag.secured.raw_meat===25&&horseBag.materialBag.rejected.raw_meat===5&&simulated.startingMaterialBag.capacity===25&&simulated.materialBagCapacity===25&&ExpeditionRules.partyProvisionCapacity(['llamrei'],null)===30&&ExpeditionRules.partyProvisionCapacity([],null)===20&&generic===22&&MaterialRules.effectiveCapacity({playerCharacter:{}})===10; })()",
            "Authored Material Bag capacity was not shared by normal creation, companion bonuses, simulation, provision parity, or legacy fallback",
        )
        check(
        ...
        check(
            "(() => { const passiveLess={...ITEM_DEFINITIONS.thorn_of_the_dolorous_vale,effects:{...ITEM_DEFINITIONS.thorn_of_the_dolorous_vale.effects}}; delete passiveLess.effects.combatSpeed; delete passiveLess.effects.onHitEffects; return Number.isFinite(EquipmentRules.scoreItem(ITEM_DEFINITIONS.thorn_of_the_dolorous_vale)) && EquipmentRules.scoreItem(ITEM_DEFINITIONS.thorn_of_the_dolorous_vale)>EquipmentRules.scoreItem(passiveLess) && COMBAT_ENEMY_DEFINITIONS.briar_knight.lootSources.some(source=>source.tableId==='briar_knight_loot') && LOOT_TABLE_DEFINITIONS.briar_knight_loot.entries.some(entry=>entry.itemId==='thorn_of_the_dolorous_vale') && !JSON.stringify(ENCOUNTER_DEFINITIONS.briar_knight).includes('thorn_of_the_dolorous_vale') && JSON.stringify(ENCOUNTER_DEFINITIONS.bell_bearer).includes('reliquary_of_saint_lazarus') && JSON.stringify(ENCOUNTER_DEFINITIONS.barenton_fountain_ritual).includes('shard_of_the_perron') && !Object.values(SHOP_DEFINITIONS).some(shop=>JSON.stringify(shop).includes('thorn_of_the_dolorous_vale')||JSON.stringify(shop).includes('reliquary_of_saint_lazarus')||JSON.stringify(shop).includes('shard_of_the_perron')); })()",
        ...
        check(
            ...
            "Poison did not tick exactly every five stadia, cross multiple intervals, or stop after Antidote",
        )
        check(
