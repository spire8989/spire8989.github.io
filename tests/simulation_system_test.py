...
def run():
    ...
    try:
        ...
        check(
            "(() => { const passiveLess={...ITEM_DEFINITIONS.thorn_of_the_dolorous_vale,effects:{...ITEM_DEFINITIONS.thorn_of_the_dolorous_vale.effects}}; delete passiveLess.effects.combatSpeed; delete passiveLess.effects.onHitEffects; return Number.isFinite(EquipmentRules.scoreItem(ITEM_DEFINITIONS.thorn_of_the_dolorous_vale)) && EquipmentRules.scoreItem(ITEM_DEFINITIONS.thorn_of_the_dolorous_vale)>EquipmentRules.scoreItem(passiveLess) && COMBAT_ENEMY_DEFINITIONS.briar_knight.lootSources.some(source=>source.tableId==='briar_knight_loot') && LOOT_TABLE_DEFINITIONS.briar_knight_loot.entries.some(entry=>entry.itemId==='thorn_of_the_dolorous_vale') && !JSON.stringify(ENCOUNTER_DEFINITIONS.briar_knight).includes('thorn_of_the_dolorous_vale') && JSON.stringify(ENCOUNTER_DEFINITIONS.leper_knight).includes('reliquary_of_saint_lazarus') && JSON.stringify(ENCOUNTER_DEFINITIONS.barenton_fountain_ritual).includes('shard_of_the_perron') && !Object.values(SHOP_DEFINITIONS).some(shop=>JSON.stringify(shop).includes('thorn_of_the_dolorous_vale')||JSON.stringify(shop).includes('reliquary_of_saint_lazarus')||JSON.stringify(shop).includes('shard_of_the_perron')); })()",
