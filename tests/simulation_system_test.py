...
def run():
    ...
    try:
        ...
        check(
            "(() => { const player=SaveSystem.createDefaultPlayerState(); player.selectedCompanion=null; player.equippedItems.weapon='thorn_of_the_dolorous_vale'; player.equippedItems.relic='reliquary_of_saint_lazarus'; const expedition=ExpeditionRules.createExpedition(player,{companion:null,provisions:5,health:40}); const combat=CombatSystem.create(expedition,'wild_boar',{random:()=>0}); const arthur=combat.allies[0]; const effects=EquipmentRules.aggregateEquippedCombatEffects(expedition); const target=combat.enemies[0]; target.hp=100; combat.status='awaitingAction'; combat.activeActorId='arthur'; arthur.gauge=100; CombatSystem.chooseAction(combat,expedition,'attack',target.id); arthur.gauge=100; combat.status='awaitingAction'; combat.activeActorId='arthur'; CombatSystem.chooseAction(combat,expedition,'attack',target.id); return arthur.speed===15 && effects.combatSpeed===5 && effects.onHitEffects.some(effect=>effect.statusId==='bleeding'&&effect.sourceItemId==='thorn_of_the_dolorous_vale') && effects.onHitEffects.some(effect=>effect.statusId==='poisoned'&&effect.sourceItemId==='reliquary_of_saint_lazarus') && target.statuses.bleeding?.remainingActivations===3 && target.statuses.poisoned?.remainingActivations===4 && combat.events.filter(event=>event.type==='status-applied').length===4 && combat.events.filter(event=>event.type==='equipment-trigger'&&event.trigger==='onHit'&&event.applied).length===4; })()",
