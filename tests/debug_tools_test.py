...
def run() -> None:
    ...
    try:
        ...
        check(
            "(() => { const encounter={encounterLayout:{arthur:{x:0.25,y:0.75},companion1:{x:0.6,y:0.4}}}; const scene=document.createElement('div'); scene.style.cssText='position:relative;width:400px;height:225px'; scene.innerHTML=renderEncounterTravelers([{type:'mount'}],encounter); document.body.append(scene); const arthur=scene.querySelector('.arthur'); const companion=scene.querySelector('.companion'); const layout=scene.querySelector('.travelers'); const first=arthur.getBoundingClientRect(); const authored=Math.abs((first.left+first.width/2-scene.getBoundingClientRect().left)/400-0.25)<0.02&&Math.abs((first.bottom-scene.getBoundingClientRect().top)/225-0.75)<0.02&&arthur.dataset.encounterLayoutX==='0.25'&&companion.dataset.encounterLayoutY==='0.4'; const stationary=getComputedStyle(layout).transitionProperty==='none'&&getComputedStyle(layout).transform==='none'; scene.style.width='800px'; scene.style.height='450px'; const resized=arthur.getBoundingClientRect(); const responsive=Math.abs((resized.left+resized.width/2-scene.getBoundingClientRect().left)/800-0.25)<0.02&&Math.abs((resized.bottom-scene.getBoundingClientRect().top)/450-0.75)<0.02; const fallback=document.createElement('div'); fallback.innerHTML=renderEncounterTravelers([],{}); const fallbackArthur=fallback.querySelector('.arthur'); const fallbackWorks=fallbackArthur.dataset.encounterLayoutX==='0.42'&&fallbackArthur.dataset.encounterLayoutY==='0.66'; const occupied=scene.querySelectorAll('.arthur,.companion').length===2; scene.remove(); return authored&&stationary&&responsive&&fallbackWorks&&occupied; })()",
            "Encounter party layout did not use normalized authored ground anchors, fallback, responsive, or occupied slots",
        )
        check(
            "(() => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:['sir_kay'],provisions:10,random:()=>0}); expedition.status='active'; expedition.travelState='traveling'; game.expedition=expedition; game.screen='expedition'; renderExpedition(); const scene=document.querySelector('#travel-scene'); const beforeArthur=document.querySelector('#travelers .arthur').getBoundingClientRect(); const beforeScene=scene.getBoundingClientRect(); const beforeX=(beforeArthur.left+beforeArthur.width/2-beforeScene.left)/beforeScene.width; EncounterManager.force(expedition,'woodland_stream'); renderExpedition(); const arthur=document.querySelector('#travelers .arthur'); const afterX=Number(arthur.dataset.encounterLayoutX); const result=Number.isFinite(afterX)&&Math.abs(afterX-beforeX)<0.01&&Math.abs(afterX-Number(game.travelPartyVisualState?.layout?.arthur?.x))<0.0001; game.expedition=originalExpedition; game.screen=originalScreen; game.travelPartyVisualState=null; renderScreen(); return result; })()",
            "Ordinary encounters did not inherit the live travel party position",
        )
        check(
            "(() => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:['sir_kay'],provisions:10,random:()=>0}); expedition.status='active'; expedition.travelState='traveling'; expedition.direction='outbound'; game.expedition=expedition; game.screen='expedition'; renderExpedition(); const travelers=document.querySelector('#travelers'); const members=[...travelers.children]; const outbound=members[0].getBoundingClientRect().left>members[1].getBoundingClientRect().left; expedition.direction='returning'; updateTravelHud(); const returning=travelers.classList.contains('is-returning')&&getComputedStyle(travelers).flexDirection==='row-reverse'&&getComputedStyle(travelers).transform.includes('matrix'); game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return outbound&&returning; })()",
            "Travel party order did not keep Arthur as the directional leader",
        )
        check(
            "(() => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:['sir_kay','llamrei'],provisions:10,random:()=>0}); expedition.status='active'; expedition.travelState='traveling'; game.expedition=expedition; game.screen='expedition'; renderExpedition(); const travelers=document.querySelector('#travelers'); const arthur=travelers?.querySelector('.arthur'); const companions=[...travelers?.querySelectorAll('.companion')??[]]; const offsets=element=>{const value=getComputedStyle(element).translate; return value==='none'?[0,0]:value.split(' ').map(Number.parseFloat).concat([0]).slice(0,2);}; const arthurBox=arthur?.getBoundingClientRect(); const companionBoxes=companions.map(element=>element.getBoundingClientRect()); const spaced=companions.length===2&&offsets(arthur)[0]===0&&companions.every(element=>offsets(element)[0]<-12&&Math.abs(offsets(element)[1])<1)&&arthurBox&&companionBoxes.every(box=>box.left<arthurBox.left); game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return spaced; })()",
            "Travel companions did not gain spacing while Arthur's travel anchor remained fixed",
        )
        check(
            "(() => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:['sir_kay','llamrei'],provisions:10,random:()=>0}); expedition.status='active'; expedition.travelState='traveling'; game.expedition=expedition; game.screen='expedition'; renderExpedition(); const companions=[...document.querySelectorAll('#travelers > .companion')]; const offset=element=>{const value=getComputedStyle(element).translate; return value==='none'?[0,0]:value.split(' ').map(Number.parseFloat);}; const result=companions.length===2&&offset(companions[1])[0]<offset(companions[0])[0]-8; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return result; })()",
            "Companion 2 did not receive the extra back-slot travel spacing",
        )
        check(
        ...
        check(
            "(() => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:['sir_kay'],provisions:10,random:()=>0}); expedition.status='active'; game.expedition=expedition; game.screen='expedition'; startCombat(expedition,'wild_boar'); renderCombat(expedition,expedition.combat); const scene=document.querySelector('.combat-scene')?.getBoundingClientRect(); const enemy=document.querySelector('.combat-enemies .combatant'); const name=enemy?.querySelector('strong'); const hud=enemy?.querySelector('.combat-unit-hud')?.getBoundingClientRect(); const visual=enemy?.querySelector('.combat-unit-visual')?.getBoundingClientRect(); const readable=name?.textContent==='Wild Boar'&&name.scrollWidth<=name.clientWidth+1; const centered=hud&&visual&&Math.abs(hud.left+hud.width/2-(visual.left+visual.width/2))<1; const bounded=hud&&scene&&hud.left>=scene.left-1&&hud.right<=scene.right+1; const wide=hud&&visual&&hud.width>visual.width&&hud.width>=112; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return readable&&centered&&bounded&&wide; })()",
            "Combat HUD names were truncated or the wider HUD lost the character anchor",
        )
        check(
            "(async () => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:['sir_kay'],provisions:10,random:()=>0}); expedition.status='active'; expedition.travelState='traveling'; game.expedition=expedition; game.screen='expedition'; renderExpedition(); const waitFor=async predicate=>{for(let index=0;index<100&&!predicate();index+=1) await new Promise(resolve=>setTimeout(resolve,10));}; const root=document.querySelector('#travelers .arthur [data-character-sprite]'); await waitFor(()=>root?.classList.contains('is-ready')&&root?.dataset.characterRequestedSlot==='walk'); const walkScale=Number.parseFloat(getComputedStyle(root).getPropertyValue('--character-visual-scale')); expedition.travelState='paused'; updateTravelHud(); await waitFor(()=>root?.classList.contains('is-ready')&&root?.dataset.characterRequestedSlot==='idle'&&root?._characterSpriteInstance?.config.frameCount===19); const idleScale=Number.parseFloat(getComputedStyle(root).getPropertyValue('--character-visual-scale')); const result=Number.isFinite(walkScale)&&Number.isFinite(idleScale)&&walkScale>0.95&&idleScale>0.95&&Math.abs(walkScale-idleScale)<0.01; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return result; })()",
            "Travel Walk and Idle did not retain the same normalized presentation scale",
        )
        check(
        ...
        check(
            "(async () => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:[],provisions:10,random:()=>0}); expedition.status='active'; expedition.travelState='traveling'; game.expedition=expedition; game.screen='expedition'; renderExpedition(); const waitFor=async predicate=>{for(let index=0;index<150&&!predicate();index+=1) await new Promise(resolve=>setTimeout(resolve,10));}; const anchor=()=>{const box=document.querySelector('#travelers')?.getBoundingClientRect(); return box&&{left:box.left,bottom:box.bottom,width:box.width,height:box.height};}; await waitFor(()=>document.querySelector('#travelers .arthur [data-character-sprite]')?._characterSpriteInstance?.config.frameCount===14); const before=anchor(); pauseTravel(); await waitFor(()=>document.querySelector('#travelers .arthur [data-character-sprite]')?._characterSpriteInstance?.config.frameCount===19); const paused=anchor(); resumeTravel(); await waitFor(()=>document.querySelector('#travelers .arthur [data-character-sprite]')?._characterSpriteInstance?.config.frameCount===14); const resumed=anchor(); const stable=(a,b)=>a&&b&&Math.abs(a.left-b.left)<0.5&&Math.abs(a.bottom-b.bottom)<0.5&&Math.abs(a.width-b.width)<0.5&&Math.abs(a.height-b.height)<0.5; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return stable(before,paused)&&stable(paused,resumed); })()",
            "The outer traveler/world anchor moved while switching Walk and Idle",
        )
        check(
        ...
        check(
            "(async () => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:[],provisions:10,random:()=>0}); expedition.status='active'; game.expedition=expedition; game.screen='expedition'; const started=startCombat(expedition,'wild_boar'); if(!started) return false; renderCombat(expedition,expedition.combat); const waitFor=async predicate=>{for(let index=0;index<100&&!predicate();index+=1) await new Promise(resolve=>setTimeout(resolve,10));}; const sprite=document.querySelector('.combat-party .combatant.ally [data-character-sprite]'); await waitFor(()=>sprite?.classList.contains('is-ready')); const canvas=sprite?.querySelector('.character-sprite-canvas'); const visual=sprite?.closest('.combat-unit-visual'); const hud=sprite?.closest('.combatant')?.querySelector('.combat-unit-hud'); const context=canvas?.getContext('2d'); const pixels=context?.getImageData(0,0,canvas.width,canvas.height).data; let minY=canvas?.height??0; let maxY=-1; if(canvas&&pixels) for(let y=0;y<canvas.height;y+=1) for(let x=0;x<canvas.width;x+=1) if(pixels[(y*canvas.width+x)*4+3]>8){minY=Math.min(minY,y);maxY=Math.max(maxY,y);} const scene=document.querySelector('.combat-scene')?.getBoundingClientRect(); const box=canvas?.getBoundingClientRect(); const visualBox=visual?.getBoundingClientRect(); const hudBox=hud?.getBoundingClientRect(); const identity=getComputedStyle(sprite).transform==='none'||getComputedStyle(sprite).transform==='matrix(1, 0, 0, 1, 0, 0)'; const result=sprite?.classList.contains('is-ready')&&identity&&getComputedStyle(sprite).overflow==='visible'&&maxY>=canvas.height-2&&minY<=1&&box?.top>=scene?.top-1&&box?.bottom<=scene?.bottom+1&&visualBox?.height>70&&hudBox?.bottom<=visualBox?.top+1; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return result; })()",
            "Solo Arthur combat sprite was clipped or its sprite root still inherited text overflow",
        )
        check(
            "(async () => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:['sir_kay'],provisions:10,random:()=>0}); expedition.status='active'; game.expedition=expedition; game.screen='expedition'; const started=startCombat(expedition,'wild_boar'); if(!started) return false; renderCombat(expedition,expedition.combat); const waitFor=async predicate=>{for(let index=0;index<100&&!predicate();index+=1) await new Promise(resolve=>setTimeout(resolve,10));}; const alphaBounds=canvas=>{const context=canvas?.getContext('2d'); if(!canvas||!context||!canvas.width||!canvas.height) return null; const pixels=context.getImageData(0,0,canvas.width,canvas.height).data; let minX=canvas.width,minY=canvas.height,maxX=-1,maxY=-1; for(let y=0;y<canvas.height;y+=1) for(let x=0;x<canvas.width;x+=1) if(pixels[(y*canvas.width+x)*4+3]>8){minX=Math.min(minX,x);minY=Math.min(minY,y);maxX=Math.max(maxX,x);maxY=Math.max(maxY,y);} return maxX>=0?{minX,minY,maxX,maxY}:null;}; const allies=[...document.querySelectorAll('.combat-party .combatant.ally')]; const arthur=allies.find((unit)=>unit.dataset.combatantId==='arthur'); const kay=allies.find((unit)=>unit.dataset.combatantId==='sir_kay'); await waitFor(()=>arthur?.querySelector('[data-character-sprite]')?.classList.contains('is-ready')); const sprite=arthur?.querySelector('[data-character-sprite]'); const canvas=sprite?.querySelector('.character-sprite-canvas'); const visual=arthur?.querySelector('.combat-unit-visual'); const hud=arthur?.querySelector('.combat-unit-hud'); const gauge=arthur?.querySelector('.gauge-bar'); const kayVisual=kay?.querySelector('.combat-unit-visual'); const canvasBounds=alphaBounds(canvas); const canvasBox=canvas?.getBoundingClientRect(); const visualBox=visual?.getBoundingClientRect(); const kayBox=kayVisual?.getBoundingClientRect(); const hudBox=hud?.getBoundingClientRect(); const gaugeBox=gauge?.getBoundingClientRect(); const fallback=kay?.querySelector('.combat-visual-fallback'); const fallbackSize=Number.parseFloat(getComputedStyle(fallback).fontSize); const visualHeight=Number.parseFloat(getComputedStyle(visual).height); const scene=document.querySelector('.combat-scene'); const sceneBox=scene?.getBoundingClientRect(); const sceneRatio=scene&&Math.abs(sceneBox.width/sceneBox.height-1.2)<0.03; const bodyVisible=canvasBounds&&canvasBounds.minY<=1&&canvasBounds.maxY>=canvas.height-2&&canvasBounds.minX>=0&&canvasBounds.maxX<canvas.width; const identity=getComputedStyle(sprite).transform==='none'||getComputedStyle(sprite).transform==='matrix(1, 0, 0, 1, 0, 0)'; const spriteVisible=identity&&getComputedStyle(sprite).overflow==='visible'&&canvasBox?.top>=sceneBox.top-1&&canvasBox?.bottom<=sceneBox.bottom+1; const result=allies.length===2&&sprite?.classList.contains('is-ready')&&bodyVisible&&spriteVisible&&sceneRatio&&kay?.querySelector('.combat-visual-fallback')?.classList.contains('is-visible')&&gaugeBox.top>=hudBox.top&&gaugeBox.bottom<=hudBox.bottom&&hudBox.top>=sceneBox.top&&hudBox.bottom<=sceneBox.bottom&&hudBox.bottom<=visualBox.top+1&&canvasBox.bottom<=kayBox.top+1&&fallbackSize<visualHeight; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return result; })()",
            "Arthur and fallback Sir Kay did not remain separated in the bounded two-ally combat layout",
        )
        check(
            "(async () => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:['sir_kay'],provisions:10,random:()=>0}); expedition.status='active'; game.expedition=expedition; game.screen='expedition'; const started=startCombat(expedition,'wild_boar'); if(!started) return false; renderCombat(expedition,expedition.combat); const scene=document.querySelector('.combat-scene'); const sprite=scene?.querySelector('.combat-party .combatant.ally [data-character-sprite]'); const waitFor=async predicate=>{for(let index=0;index<100&&!predicate();index+=1) await new Promise(resolve=>setTimeout(resolve,10));}; await waitFor(()=>sprite?.classList.contains('is-ready')&&sprite?._characterSpriteInstance?.config.frameCount===19); const instance=sprite?._characterSpriteInstance; const sceneBefore=scene; const firstFrame=instance?.frameIndex; await new Promise(resolve=>setTimeout(resolve,420)); const secondFrame=sprite?._characterSpriteInstance?.frameIndex; const hud=scene?.querySelector('.combat-party .combatant.ally .combat-unit-hud')?.getBoundingClientRect(); const visual=scene?.querySelector('.combat-party .combatant.ally .combat-unit-visual')?.getBoundingClientRect(); const gauge=scene?.querySelector('.combat-party .combatant.ally .gauge-bar')?.getBoundingClientRect(); const sceneBox=scene?.getBoundingClientRect(); const identity=getComputedStyle(sprite).transform==='none'||getComputedStyle(sprite).transform==='matrix(1, 0, 0, 1, 0, 0)'; const result=instance?.config.frameCount===19&&instance?.config.columns===5&&identity&&firstFrame!==secondFrame&&document.querySelector('.combat-scene')===sceneBefore&&hud.top>=sceneBox.top&&hud.bottom<=sceneBox.bottom&&gauge.top>=hud.top&&gauge.bottom<=hud.bottom&&hud.bottom<=visual.top+1&&visual.bottom<=sceneBox.bottom+1; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return result; })()",
            "Arthur's combat Idle animation did not continue with the HUD above the real visual region",
        )
