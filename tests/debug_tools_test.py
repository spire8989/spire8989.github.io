...
def run() -> None:
    ...
    try:
        ...
        check(
            "(() => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:['sir_kay'],provisions:10,random:()=>0}); expedition.status='active'; game.expedition=expedition; game.screen='expedition'; const started=startCombat(expedition,'wild_boar'); if(!started) return false; renderCombat(expedition,expedition.combat); const allies=[...document.querySelectorAll('.combat-party .combatant.ally')]; const arthur=allies.find((unit)=>unit.dataset.combatantId==='arthur'); const kay=allies.find((unit)=>unit.dataset.combatantId==='sir_kay'); const visual=arthur?.querySelector('.combat-unit-visual'); const hud=arthur?.querySelector('.combat-unit-hud'); const gauge=arthur?.querySelector('.gauge-bar'); const kayHud=kay?.querySelector('.combat-unit-hud'); const kayBox=kay?.getBoundingClientRect(); const visualBox=visual?.getBoundingClientRect(); const hudBox=hud?.getBoundingClientRect(); const gaugeBox=gauge?.getBoundingClientRect(); const kayHudBox=kayHud?.getBoundingClientRect(); const fallback=kay?.querySelector('.combat-visual-fallback'); const fallbackSize=Number.parseFloat(getComputedStyle(fallback).fontSize); const visualHeight=Number.parseFloat(getComputedStyle(visual).height); const result=allies.length===2&&arthur?.querySelector('[data-character-sprite]')?.classList.contains('is-ready')&&kay?.querySelector('.combat-visual-fallback')?.classList.contains('is-visible')&&hudBox.bottom<=visualBox.top+1&&visualBox.bottom<=gaugeBox.top+1&&visualBox.bottom<=kayBox.top+1&&visualBox.bottom<=kayHudBox.top+1&&fallbackSize<visualHeight; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return result; })()",
