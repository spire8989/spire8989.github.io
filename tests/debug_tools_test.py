...
def run() -> None:
    ...
    try:
        ...
        check(
            "(async () => { const host=document.createElement('div'); host.innerHTML=renderCharacterSprite(PLAYER_CHARACTER_DEFINITION,'walk','travel','fallback','Arthur'); document.body.append(host); const root=host.querySelector('[data-character-sprite]'); const image=root?.querySelector('.character-sprite-source'); const canvas=root?.querySelector('.character-sprite-canvas'); const waitFor=async predicate=>{for(let index=0;index<100&&!predicate();index+=1) await new Promise(resolve=>setTimeout(resolve,10));}; await waitFor(()=>root?.classList.contains('is-ready')&&canvas?.width>0); const walkReady=root?.classList.contains('is-ready')&&canvas?.width<256&&canvas?.height<256&&canvas?.height>0&&!root?.querySelector('.character-sprite-fallback')?.classList.contains('is-visible'); setCharacterVisualState(root,'idle'); await waitFor(()=>root?.dataset.characterRequestedSlot==='idle'&&root?.classList.contains('is-ready')&&canvas?.height>0); const idleReady=root?.dataset.characterRequestedSlot==='idle'&&root?.classList.contains('is-ready')&&canvas?.width<768&&canvas?.height<768&&!root?.querySelector('.character-sprite-fallback')?.classList.contains('is-visible'); host.remove(); return walkReady&&idleReady&&image?.dataset.assetId==='combat_arthur_idle_basesprite'; })()",
