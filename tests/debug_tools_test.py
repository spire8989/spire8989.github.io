...
def run() -> None:
    ...
    try:
        ...
        check("document.querySelector('#debug-town-hotspot-style')?.value==='plaque'", "Town hotspot style switcher did not default to plaque")
        check(
            "(() => { game.screen='location'; renderLocation(); const before=[...document.querySelectorAll('.hub-hotspot')].map(button=>`${button.dataset.destinationId}:${button.dataset.hotspotX},${button.dataset.hotspotY}`).join('|'); let style=document.querySelector('#debug-town-hotspot-style'); style.value='label'; style.dispatchEvent(new Event('change',{bubbles:true})); const labels=[...document.querySelectorAll('.hub-hotspot')].every(button=>button.classList.contains('town-hotspot-style-label')&&button.textContent.includes(button.querySelector('strong')?.textContent||'')); style=document.querySelector('#debug-town-hotspot-style'); style.value='seal'; style.dispatchEvent(new Event('change',{bubbles:true})); const seals=[...document.querySelectorAll('.hub-hotspot')].every(button=>button.classList.contains('town-hotspot-style-seal')); const after=[...document.querySelectorAll('.hub-hotspot')].map(button=>`${button.dataset.destinationId}:${button.dataset.hotspotX},${button.dataset.hotspotY}`).join('|'); game.screen='campaign'; renderScreen(); return labels&&seals&&before===after; })()",
            "Town hotspot style switching changed coordinates or failed to update marker classes",
        )
