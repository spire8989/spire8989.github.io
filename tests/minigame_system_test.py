...
def run():
    ...
    try:
        ...
        check(
            ...
            " const plungeMotion=(()=>{const visual=document.querySelector('.fishing-bobber-visual'); const style=getComputedStyle(visual); return style.animationName.includes('fishing-bobber-hook') && style.animationIterationCount==='infinite' && document.querySelector('.fishing-stage-hint')?.textContent==='BITE!';})();"
            ...
            " && document.querySelector('.fishing-reward-line').getBoundingClientRect().bottom<=document.querySelector('.fishing-result-card .game-button').getBoundingClientRect().top;"
            ...
            " s.activeCast.remainingMs=0; updateFishing(0.1); s.hookRemainingMs=0; updateFishing(0.8); return s.state==='result' && !document.querySelector('.fishing-bobber') && document.querySelector('.fishing-result-card')?.textContent.includes('The fish got away.');})();"
            " dismissFishingResult();"
