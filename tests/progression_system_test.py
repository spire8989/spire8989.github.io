...
def run() -> None:
    ...
    try:
        ...
        check(
            "(() => { const makeFinal=()=>{const p=SaveSystem.createDefaultPlayerState(); p.learnedKnowledge=[...p.learnedKnowledge,'song_of_the_forest']; p.campaignFlags={...p.campaignFlags,forest_village_discovered:true,druid_favor_complete:true}; p.ownedItems={...p.ownedItems,verdant_shard_grace:1,verdant_shard_wrath:1,enchanted_verdant_heart:1}; p.currentGold=1000; p.provisions=500; p.provisionStock=500; return p;}; const originalMinimum=ENCOUNTER_DEFINITIONS.verdant_altar.minimumDistance; const originalMaximum=ENCOUNTER_DEFINITIONS.verdant_altar.maximumDistance; let observed; try { const p=makeFinal(); const before=assessOldForestProgressionGoal(p,{strategy:'cautious'}); ENCOUNTER_DEFINITIONS.verdant_altar.minimumDistance=205; ENCOUNTER_DEFINITIONS.verdant_altar.maximumDistance=213; const after=assessOldForestProgressionGoal(p,{strategy:'cautious'}); const c=CampaignSimulationRunner.run({seed:'authored-altar-distance',campaignMode:'progression',completionObjective:'full_campaign',expeditions:1,expeditionPlan:[180],strategy:'normal',healingEnabled:false,autoSellRecoveredLoot:false,startingState:p}); const e=c.expeditions[0]; observed=before.targetDistance===originalMinimum&&after.goalId==='defeat-verdant-warden'&&after.targetDistance===205&&after.minimumAttemptDistance===205&&e?.configuredTargetDistance===180&&e?.desiredTargetDistance===205&&e?.oldForestTargetMilestoneDistance===205; } finally { ENCOUNTER_DEFINITIONS.verdant_altar.minimumDistance=originalMinimum; ENCOUNTER_DEFINITIONS.verdant_altar.maximumDistance=originalMaximum; } return observed; })()",
        ...
        check(
            "(() => { const originalMinimum=ENCOUNTER_DEFINITIONS.thorn_crowned_hart.minimumDistance; const originalMaximum=ENCOUNTER_DEFINITIONS.thorn_crowned_hart.maximumDistance; try { ENCOUNTER_DEFINITIONS.thorn_crowned_hart.minimumDistance=155; ENCOUNTER_DEFINITIONS.thorn_crowned_hart.maximumDistance=163; const p=SaveSystem.createDefaultPlayerState(); p.learnedKnowledge=[...p.learnedKnowledge,'song_of_the_forest']; p.campaignFlags={...p.campaignFlags,forest_village_discovered:true,druid_favor_complete:true}; p.ownedItems={...p.ownedItems,verdant_shard_grace:1}; const goal=assessOldForestProgressionGoal(p,{strategy:'cautious'}); return goal.goalId==='secure-wrath-shard'&&goal.targetDistance===155&&goal.minimumAttemptDistance===155&&goal.requiredPreparation.encounter==='thorn_crowned_hart'&&goal.requiredPreparation.targetDistance===155; } finally { ENCOUNTER_DEFINITIONS.thorn_crowned_hart.minimumDistance=originalMinimum; ENCOUNTER_DEFINITIONS.thorn_crowned_hart.maximumDistance=originalMaximum; } })()",
            "Thorn-Crowned Hart progression did not resolve from its authored encounter distance",
        )
        check(
            "(() => { const originalWhite=ENCOUNTER_DEFINITIONS.white_hart.minimumDistance; const originalWhiteMaximum=ENCOUNTER_DEFINITIONS.white_hart.maximumDistance; const originalVillage=ENCOUNTER_DEFINITIONS.hidden_forest_village.minimumDistance; const originalVillageMaximum=ENCOUNTER_DEFINITIONS.hidden_forest_village.maximumDistance; try { ENCOUNTER_DEFINITIONS.white_hart.minimumDistance=61; ENCOUNTER_DEFINITIONS.white_hart.maximumDistance=69; ENCOUNTER_DEFINITIONS.hidden_forest_village.minimumDistance=123; ENCOUNTER_DEFINITIONS.hidden_forest_village.maximumDistance=131; const p=SaveSystem.createDefaultPlayerState(); p.learnedKnowledge=[...p.learnedKnowledge,'woodcraft']; const grace=assessOldForestProgressionGoal(p,{strategy:'cautious'}); p.ownedItems={...p.ownedItems,verdant_shard_grace:1}; const village=assessOldForestProgressionGoal(p,{strategy:'cautious'}); return grace.goalId==='secure-grace-shard'&&grace.targetDistance===61&&grace.minimumAttemptDistance===61&&grace.requiredPreparation.encounter==='white_hart'&&grace.requiredPreparation.targetDistance===61&&village.goalId==='discover-village'&&village.targetDistance===123&&village.requiredPreparation.targetDistance===123; } finally { ENCOUNTER_DEFINITIONS.white_hart.minimumDistance=originalWhite; ENCOUNTER_DEFINITIONS.white_hart.maximumDistance=originalWhiteMaximum; ENCOUNTER_DEFINITIONS.hidden_forest_village.minimumDistance=originalVillage; ENCOUNTER_DEFINITIONS.hidden_forest_village.maximumDistance=originalVillageMaximum; } })()",
            "Earlier encounter-backed Old Forest milestones did not follow authored distances",
        )
        check(
            "(() => { const original=ENCOUNTER_DEFINITIONS.verdant_altar.minimumDistance; try { ENCOUNTER_DEFINITIONS.verdant_altar.minimumDistance=undefined; try { resolveProgressionEncounterDistance('verdant_altar'); return false; } catch (error) { return error.message.includes('invalid authored distance'); } } finally { ENCOUNTER_DEFINITIONS.verdant_altar.minimumDistance=original; } })()",
