...
def run() -> None:
    ...
    try:
        ...
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'progression-early-old',campaignMode:'progression',expeditions:1,turnaroundDistance:101,startingState:{arthurHealth:45,currentGold:0,provisions:15}}); const e=c.expeditions[0]; return e.isSupplyRun&&e.supplyRunForRoute==='old_forest_road'&&!c.routesCompleted.includes('old_forest_road')&&c.attemptsByRoute.old_forest_road===0&&c.stopReason==='progression-attempt-cap'&&!c.completedPlan; })()",
            "An underprepared Old Forest progression route launched a fake shortened attempt",
        )
        check(
            "(() => { const p=SaveSystem.createDefaultPlayerState(); p.currentGold=100; p.provisions=10; const d=applyBetweenExpeditionPolicy(p,CampaignRules.createShopStocks(),BetweenExpeditionPolicies['aggressive-reinvestor'],105,false,'aggressive',()=>0.1,[],{progressionRequiredDistance:105}); const buffer=d.strategyConstraints.find(entry=>entry.type==='preferred-provision-buffer-unavailable'); return d.actualTargetDistance===105&&d.provisionsToPack===30&&d.minimumViableProvisionRequirement<=30&&d.preferredProvisionTarget===31&&d.preferredBufferShortfall===1&&d.targetDistanceReduced===false&&d.targetDistanceReduction===0&&d.progressionReadiness==='ready-with-constraints'&&buffer?.preferredBufferShortfall===1; })()",
            "Aggressive progression planning reduced a viable 105-league objective for its optional buffer",
        )
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'probe-aggressive',campaignMode:'progression',expeditions:1,strategy:'aggressive',betweenExpeditionPolicy:'aggressive-reinvestor',turnaroundDistance:105,startingState:{currentGold:100,provisions:30}}); const e=c.expeditions[0]; return e.routeId==='old_forest_road'&&!e.isSupplyRun&&e.actualTargetDistance===105&&!e.targetDistanceReduced&&e.actualMaximumDistance>=105&&e.routeAttemptCompleted&&e.progressionRequiredDistance===105&&e.minimumViableProvisionRequirement<=e.actualPackedProvisions; })()",
            "A viable aggressive Old Forest objective did not preserve its 105-league floor through completion",
        )
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'probe-aggressive',campaignMode:'progression',expeditions:1,strategy:'aggressive',betweenExpeditionPolicy:'aggressive-reinvestor',turnaroundDistance:105,startingState:{currentGold:100,provisions:30}}); const compact=JSON.parse(CampaignSimulationTelemetry.toCompactJson({results:[c]})); const planning=compact.campaigns[0].expeditions[0].planning; return planning.progressionRequiredDistance===105&&planning.minimumViableProvisionRequirement<=30&&planning.preferredProvisionTarget===31&&planning.provisionCapacity===30&&planning.actualPackedProvisions===30&&planning.preferredBufferShortfall===1&&planning.supplyRunExpectedBenefit===false; })()",
            "Compact progression telemetry omitted the minimum/preferred provision distinction",
        )
        check(
            "(() => { const readiness=assessProgressionReadiness('synthetic_objective_route',120,120,(() => { const p=SaveSystem.createDefaultPlayerState(); p.currentGold=0; p.provisions=0; return p; })(),(() => { const s=CampaignRules.createShopStocks(); s.village_general_goods=0; return s; })(),BetweenExpeditionPolicies['conservative-sustainer'],'cautious',{supplyRunHistoryByRoute:{'synthetic_objective_route':[{materiallyImproved:false}]}}); return readiness.status==='blocked'&&!readiness.supplyRunExpectedBenefit&&readiness.supplyRunBenefitReason==='supply-run-no-material-benefit'&&readiness.reason==='supply-run-no-material-benefit'; })()",
            "A repeated supply route was scheduled despite no material readiness improvement",
        )
        check(
        ...
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'merlin-cautious',campaignMode:'progression',expeditions:20,strategy:'cautious',betweenExpeditionPolicy:'conservative-sustainer',turnaroundDistance:101,startingState:{arthurHealth:45,currentGold:3000,provisions:100,ownedItems:{flask:1},packedItems:['flask']}}); return c.currentContentCompleted&&c.completedPlan&&c.stopReason==='current-content-completed'&&c.expeditionsAttempted===5&&c.morgansTokenSecured&&c.merlinFound&&c.routesCompleted.join(',')==='old_forest_road,fountain_of_barenton,val_sans_retour,search_for_merlin'&&c.endingState.ownedItems.merlins_seal===1; })()",
            "Merlin's Search did not complete and stop the current campaign after discovery-gated Barenton/Val progression",
        )
        check(
        ...
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'merlin-configured-105',campaignMode:'progression',expeditions:20,strategy:'cautious',betweenExpeditionPolicy:'conservative-sustainer',expeditionPlan:[105],startingState:{arthurHealth:45,currentGold:3000,provisions:100,ownedItems:{flask:1},packedItems:['flask']}}); const search=c.expeditions.find(entry=>entry.routeId==='search_for_merlin'&&!entry.isSupplyRun); return search&&search.configuredTargetDistance===105&&search.desiredTargetDistance===120&&search.progressionRequiredDistance===120&&search.actualTargetDistance>=120; })()",
            "The configured 105 target did not rise to the generic 120 objective floor",
        )
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'merlin-cautious',campaignMode:'progression',expeditions:20,strategy:'cautious',betweenExpeditionPolicy:'conservative-sustainer',turnaroundDistance:101,startingState:{arthurHealth:45,currentGold:3000,provisions:100,ownedItems:{flask:1},packedItems:['flask']}}); const search=c.expeditions.filter(entry=>entry.campaignStageAtDeparture==='search_for_merlin'); const supply=search.filter(entry=>entry.isSupplyRun); const attempt=search.find(entry=>!entry.isSupplyRun); const validSupply=supply.every(entry=>entry.runKind==='supply'&&entry.actualTargetDistance<120&&entry.routeAttemptNumber===null&&entry.progressionReadiness==='deferred'&&entry.progressionDeferredReason==='objective-distance-floor'); return attempt&&validSupply&&attempt.actualTargetDistance>=120&&c.progressionDeferredCount===supply.length&&c.objectiveDistanceFloorViolations===0&&c.currentContentCompleted; })()",
            "Cautious did not preserve the Merlin objective floor while using any required preparation runs",
        )
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'merlin-cautious',campaignMode:'progression',expeditions:20,strategy:'cautious',betweenExpeditionPolicy:'conservative-sustainer',turnaroundDistance:101,startingState:{arthurHealth:45,currentGold:3000,provisions:100,ownedItems:{flask:1},packedItems:['flask']}}); const search=c.expeditions.filter(entry=>entry.campaignStageAtDeparture==='search_for_merlin'&&!entry.isSupplyRun); return search.length===1&&search[0].actualTargetDistance>=search[0].routeObjectiveDistance&&['ready','ready-with-constraints'].includes(search[0].progressionReadiness)&&c.objectiveDistanceFloorViolations===0; })()",
            "Cautious progression did not preserve the objective floor for the Merlin attempt",
        )
        check(
        ...
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'merlin-cautious',campaignMode:'progression',expeditions:20,strategy:'cautious',betweenExpeditionPolicy:'conservative-sustainer',turnaroundDistance:101,startingState:{arthurHealth:45,currentGold:3000,provisions:100,ownedItems:{flask:1},packedItems:['flask']}}); const compact=JSON.parse(CampaignSimulationTelemetry.toCompactJson({results:[c]})); const campaign=compact.campaigns[0]; const deferred=campaign.expeditions.filter(entry=>entry.isSupplyRun&&entry.campaignStageAtDeparture==='search_for_merlin'); const deferralsValid=deferred.length===0||deferred.every(entry=>entry.planning.progressionReadiness==='deferred'&&entry.planning.actualTargetDistance<entry.planning.routeObjectiveDistance)&&campaign.notableEvents.some(event=>event.type==='progression-deferred'&&event.supplyRun); return campaign.campaignSummary.progression.progressionDeferredCount===deferred.length&&campaign.campaignSummary.progression.objectiveDistanceFloorViolations===0&&deferralsValid; })()",
        ...
        check(
            "(() => { const batch=CampaignSimulationRunner.runBatch({scenarios:[{id:'progression-funnel',seed:'progression-funnel',campaignMode:'progression',expeditions:10,strategy:'cautious',turnaroundDistance:101,startingState:{arthurHealth:45,currentGold:3000,provisions:100,ownedItems:{flask:1},packedItems:['flask']}}],campaignsPerScenario:2}); const s=batch.summary; return s.totalCampaigns===2&&s.oldForestReachedRate===1&&s.oldForestCompletionRate===1&&s.barentonReachedRate===1&&s.barentonCompletionRate===1&&s.valReachedRate===1&&s.valCompletionRate===1&&s.averageAttemptsBarenton>=1&&s.averageProgressionDeferredCount>=0&&s.averageObjectiveDistanceFloorViolations===0&&s.deathsByRoute&&s.deathsByRoute.search_for_merlin!==undefined&&s.averageEncounterCountByRoute&&s.barentonCompletionAttemptDistribution&&s.valCompletionAttemptDistribution&&s.averageBarentonDiscoveryReturnRate>0&&s.averageValDiscoveryReturnRate>0&&s.averageMorganOfferReached>0&&s.averageGuardianReached>0; })()",
        ...
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'progression-replay-route-switch',campaignMode:'progression',expeditions:3,strategy:'cautious',turnaroundDistance:101,startingState:{arthurHealth:45,currentGold:3000,provisions:100,ownedItems:{flask:1},packedItems:['flask']}}); const data=CampaignReplayData.normalize(c); return data.routeSequence.join(',')===c.routeSequence.join(',')&&data.progressionTransitions.length===c.progressionTransitions.length&&data.townActions.some(a=>a.type==='select-expedition'&&a.expeditionId==='fountain_of_barenton')&&data.expeditions.map(e=>e.replay.expeditionId).join(',')===c.routeSequence.join(','); })()",
