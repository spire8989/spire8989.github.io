...
def run() -> None:
    ...
    try:
        ...
        check(
            "(() => { const c=CampaignSimulationRunner.run({seed:'merlin-cautious',campaignMode:'progression',expeditions:20,strategy:'cautious',betweenExpeditionPolicy:'conservative-sustainer',turnaroundDistance:101,startingState:{arthurHealth:45,currentGold:3000,provisions:100,ownedItems:{flask:1},packedItems:['flask']}}); const search=c.expeditions.filter(entry=>entry.campaignStageAtDeparture==='search_for_merlin'&&!entry.isSupplyRun); return search.length===1&&search[0].actualTargetDistance>=search[0].routeObjectiveDistance&&search[0].progressionReadiness==='ready'&&c.objectiveDistanceFloorViolations===0; })()",
