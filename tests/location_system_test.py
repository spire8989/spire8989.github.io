...
def run():
    ...
    try:
        ...
        check("game.player.saveVersion === 8 && game.player.learnedRecipes.includes('bandages') && game.player.materials.cloth === 3", "Fresh crafting state is invalid")
        check("(() => { const migrated=sanitizePlayerState({saveVersion:6,materials:{iron:2,unknown:9},learnedRecipes:['antidote','unknown']},SaveSystem.createDefaultPlayerState()); return migrated.saveVersion===8&&migrated.materials.iron===2&&!migrated.materials.unknown&&migrated.learnedRecipes.length===1&&migrated.learnedRecipes[0]==='antidote'&&migrated.campaignFlags.broceliande_intro_complete; })()", "Crafting save migration did not sanitize stable IDs")
        check("(() => { const player=SaveSystem.createDefaultPlayerState(); const expedition=ExpeditionRules.createExpedition(player,{random:()=>0}); const reward=LootRules.resolveTable('forest_materials',{player,expedition,random:()=>0,debugLog:expedition.lootDebugLog}); return reward.materialId==='medicinal_herbs'&&expedition.unsecuredMaterials.medicinal_herbs===1&&expedition.lootDebugLog.some(event=>event.type==='loot-selected'); })()", "Nested material loot did not resolve into staged expedition state")
        ...
        check("(() => { const originalPacked=[...game.player.packedItems]; const originalOwned={...game.player.ownedItems}; const candidates=Object.values(ITEM_DEFINITIONS).filter(item=>item.carriable&&!Object.values(game.player.equippedItems).includes(item.id)); if(candidates.length<7)return false; candidates.forEach(item=>{game.player.ownedItems[item.id]=Math.max(1,game.player.ownedItems[item.id]||0);}); game.player.packedItems=candidates.slice(0,6).map(item=>item.id); renderPreparation(); const extra=candidates[6]; const disabled=document.querySelector(`[data-action=\"toggle-pack-item\"][data-item-id=\"${extra.id}\"]`)?.disabled; game.player.packedItems=originalPacked; game.player.ownedItems=originalOwned; renderPreparation(); return disabled===true; })()", "Unified preparation screen did not enforce the existing pack slot limit")
        for width, height in ((320, 480), (360, 640), (390, 844), (430, 932)):
            devtools.call("Emulation.setDeviceMetricsOverride", {
                "width": width,
                "height": height,
        ...
        check("(() => { const description=DESTINATION_DEFINITIONS.inn.description; const first=document.querySelector('.interaction-scroll > :first-child'); return document.querySelector('.destination-visual')?.textContent.includes(description) && !document.querySelector('.interaction-scroll')?.textContent.includes(description) && document.querySelector('.npc-card')?.textContent.includes('Innkeeper') && document.querySelector('#destination-title')?.textContent==='The Inn' && getComputedStyle(first).marginTop==='0px'; })()", "Inn duplicated its atmospheric description or retained an unnecessary content gap")
        ...
        check("(() => { const description=DESTINATION_DEFINITIONS.merchant.description; const first=document.querySelector('.interaction-scroll > :first-child'); return document.querySelector('.destination-visual')?.textContent.includes(description) && !document.querySelector('.interaction-scroll')?.textContent.includes(description) && document.querySelector('.shopkeeper-row')?.textContent.includes('Village Merchant') && document.querySelector('#destination-title')?.textContent==='Merchant' && getComputedStyle(first).marginTop==='0px'; })()", "Merchant duplicated its atmospheric description or retained an unnecessary content gap")
        ...
        check("document.body.textContent.includes('Repair Kit') && document.body.textContent.includes('Iron 2/2')", "Blacksmith crafting view is missing known recipe costs")
        ...
        check("(() => { const description=DESTINATION_DEFINITIONS.apothecary.description; return document.querySelector('.destination-visual')?.textContent.includes(description) && !document.querySelector('.interaction-scroll')?.textContent.includes(description) && document.querySelector('.shopkeeper-row')?.textContent.includes('Apothecary') && document.querySelector('#destination-title')?.textContent==='Apothecary'; })()", "Apothecary duplicated its atmospheric description or lost its service card")
        ...
        check("Boolean(document.querySelector('.journey-controls') && document.querySelector('.expedition-status') && document.querySelector('.expedition-details') && document.querySelector('.expedition-action-bar')) && !document.querySelector('.strategic-exit') && !document.querySelector('.destructive-exit') && !document.querySelector('.journey-state-banner') && !document.querySelector('.journey-heading h2') && !document.querySelector('.expedition-details')?.open && document.querySelector('.journey-summary')?.textContent.includes('pace') && !document.querySelector('.journey-summary')?.textContent.includes('food rate') && document.querySelector('#return-button')?.textContent.includes('Return Journey')", "Expedition HUD did not simplify status, Journey, details, and sticky actions")
        ...
        devtools.click('.run-details-summary')
        check("document.querySelector('.expedition-details')?.open", "Expedition Details did not expand")
        check("document.querySelector('.run-details-content')?.textContent.includes('Company')", "Expanded Expedition Details did not retain its readable content")
        check("Boolean(document.querySelector('.run-details-actions [data-action=\"abandon-expedition\"]'))", "Expanded Expedition Details did not contain the abandon action")
        devtools.click('.run-details-summary')
        ...
        check("document.querySelector('.unsecured-card')?.getBoundingClientRect().width >= document.querySelector('.resource-grid').getBoundingClientRect().width - 2 && getComputedStyle(document.querySelector('.unsecured-card')).gridColumn === '1 / -1'", "Unsecured Loot did not become a full-width expedition bar")
        check("!document.querySelector('#loot-empty-state') && document.querySelector('.unsecured-card')?.textContent.includes('Unsecured Loot')", "Unsecured Loot zero state did not collapse to compact counts")
        ...
        check("document.querySelectorAll('.run-detail-collection').length === 3 && [...document.querySelectorAll('.run-item-chip')].every(chip => getComputedStyle(chip).whiteSpace !== 'nowrap')", "Expedition loadout/carried/discovery collections do not use wrapping chips")
