...
def run():
    ...
    try:
        ...
        check("document.querySelector('.expedition-details .run-details-content')?.textContent.includes('Company')", "Expanded Expedition Details did not retain its readable content")
        ...
        check("getComputedStyle(document.querySelector('.camp-tabs')).position==='sticky' && getComputedStyle(document.querySelector('.camp-actions')).position==='sticky'", "Camp navigation and Leave Camp are not persistent within the camp scroller")
        ...
        check(f"!game.expedition.combat && game.expedition.activeEncounter.phase==='result' && Math.abs(game.expedition.provisions-{provisions_before_victory})<0.000001 && game.expedition.activeEncounter.rewards.some(reward=>reward.materialId==='raw_meat') && document.body.textContent.includes('Raw Meat') && document.querySelector('#journey-log-preview')?.textContent.includes('Raw Meat')", "Boar victory did not expose its Raw Meat reward or Journey Log entry without inventing provisions")
        check("game.expedition.activeEncounter.resultText.includes('Raw Meat') && Boolean(document.querySelector('[data-action=\"continue-journey\"]'))", "Combat victory did not return cleanly through Continue Journey with its reward")
        devtools.click('[data-action="continue-journey"]')
        ...
        devtools.click('[data-action="combat-action"][data-combat-action-id="attack"]')
        check(f"!game.expedition.combat && game.expedition.activeEncounter.phase==='result' && Math.abs(game.expedition.provisions-{wolf_victory_provisions})<0.000001 && game.expedition.activeEncounter.rewards.some(reward=>reward.materialId==='raw_meat') && document.body.textContent.includes('Raw Meat') && document.querySelector('#journey-log-preview')?.textContent.includes('Raw Meat')", "Defeating all wolves did not expose two Raw Meat, a Journey Log entry, or resolve cleanly")
        devtools.click('[data-action="continue-journey"]')
