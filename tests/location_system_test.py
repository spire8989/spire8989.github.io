...
def run():
    ...
    try:
        ...
        check("COMBAT_TUNING.actionGaugeMaximum === 100 && COMBAT_TUNING.fleeChance === 0.7", "Combat tuning is not centralized")
        check("(() => { const sample={...game.expedition,selectedCompanion:null,health:73,companionCombatHp:{}}; const combat=CombatSystem.create(sample,'wild_boar',{random:()=>0}); return combat.allies.length===1 && combat.allies[0].id==='arthur' && combat.allies[0].hp===73 && !JSON.stringify(combat).includes('sir_kay'); })()", "Solo combat incorrectly includes Kay or loses Arthur health")
        check("(() => { const sample={...game.expedition,selectedCompanion:'sir_kay',health:100,companionCombatHp:{sir_kay:41}}; const combat=CombatSystem.create(sample,'wild_boar',{random:()=>0}); return combat.allies.length===2 && combat.allies.find(ally=>ally.id==='sir_kay').hp===41; })()", "Kay party combat or expedition HP persistence is invalid")
        check("(() => { const sample={...game.expedition,selectedCompanion:null,health:100,companionCombatHp:{}}; const combat=CombatSystem.create(sample,'wild_boar',{random:()=>0}); CombatSystem.update(combat,sample,8); const gauges=combat.allies[0].gauge+combat.enemies[0].gauge; CombatSystem.update(combat,sample,5); return combat.status==='awaitingAction' && combat.activeActorId==='arthur' && combat.allies[0].gauge+combat.enemies[0].gauge===gauges; })()", "Ready gauges do not pause while awaiting player input")
        check("(() => { const sample={...game.expedition,selectedCompanion:null,health:100,companionCombatHp:{}}; const combat=CombatSystem.create(sample,'wild_boar',{random:()=>0}); combat.allies[0].gauge=100; CombatSystem.update(combat,sample,0); const before=combat.enemies[0].hp; CombatSystem.chooseAction(combat,sample,'attack'); return combat.enemies[0].hp===before-7; })()", "Arthur's equipped attack did not damage the boar using defense")
        check("(() => { const sample={...game.expedition,selectedCompanion:'sir_kay',health:100,companionCombatHp:{}}; const combat=CombatSystem.create(sample,'wild_boar',{random:()=>0}); const kay=combat.allies.find(ally=>ally.id==='sir_kay'); kay.gauge=100; CombatSystem.update(combat,sample,0); CombatSystem.chooseAction(combat,sample,'intercede'); combat.enemies[0].gauge=100; CombatSystem.update(combat,sample,0); return sample.health===100 && sample.companionCombatHp.sir_kay===78 && !kay.interceding && combat.log.some(line=>line.includes('meant for Arthur')); })()", "Intercede did not redirect and persist enemy Charge damage")
        check("(() => { const sample={...game.expedition,selectedCompanion:null,health:73,companionCombatHp:{}}; const combat=CombatSystem.create(sample,'wild_boar',{random:()=>0}); const arthur=combat.allies[0]; arthur.gauge=100; CombatSystem.update(combat,sample,0); CombatSystem.chooseAction(combat,sample,'defend'); combat.enemies[0].gauge=100; CombatSystem.update(combat,sample,0); const persisted=sample.health===70 && arthur.defending; arthur.gauge=100; CombatSystem.update(combat,sample,0); return persisted && !arthur.defending; })()", "Defend mitigation, duration, or Arthur expedition health persistence is invalid")
        ...
        check("(() => { const sample={...game.expedition,selectedCompanion:'sir_kay',health:100,companionCombatHp:{}}; const combat=CombatSystem.create(sample,'wolves',{random:()=>0}); const kay=combat.allies.find(ally=>ally.id==='sir_kay'); kay.gauge=100; CombatSystem.update(combat,sample,0); CombatSystem.chooseAction(combat,sample,'intercede'); combat.enemies[1].gauge=100; CombatSystem.update(combat,sample,0); const afterFirst=sample.companionCombatHp.sir_kay; combat.enemies[2].gauge=100; CombatSystem.update(combat,sample,0); return afterFirst===84 && sample.health===99 && sample.companionCombatHp.sir_kay===84 && !kay.interceding; })()", "Kay Intercede was bypassed or consumed more than once by multiple wolves")
