...
def run():
    ...
    try:
        ...
        check("(() => { const screen=document.querySelector('.camp-screen').getBoundingClientRect(); const panel=document.querySelector('.camp-panel').getBoundingClientRect(); const footer=document.querySelector('.camp-actions'); const footerBox=footer.getBoundingClientRect(); return getComputedStyle(document.querySelector('.camp-tabs')).position==='sticky' && footer.parentElement.classList.contains('camp-screen') && getComputedStyle(footer).position==='relative' && Math.abs(footerBox.bottom-screen.bottom)<1 && Math.abs(footerBox.top-panel.bottom)<1; })()", "Camp navigation or the external Leave Camp footer is not structurally anchored to the viewport")
