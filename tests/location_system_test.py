...
def run():
    ...
    try:
        ...
        check("document.querySelectorAll('.hub-hotspot').length === 4 && !document.querySelector('[data-destination-id=\"forest_gate\"]')", "Village should expose only the four service locations")
        check("document.querySelector('[data-action=\"prepare-expedition\"]')?.textContent.includes('Prepare for Expedition') && !document.querySelector('[data-action=\"view-inventory\"]')", "Village is missing the unified preparation primary action")
        ...
        check("game.player.saveVersion === 7 && game.player.learnedRecipes.includes('bandages') && game.player.materials.cloth === 3", "Fresh crafting state is invalid")
        check("(() => { const migrated=sanitizePlayerState({saveVersion:6,materials:{iron:2,unknown:9},learnedRecipes:['antidote','unknown']},SaveSystem.createDefaultPlayerState()); return migrated.saveVersion===7&&migrated.materials.iron===2&&!migrated.materials.unknown&&migrated.learnedRecipes.length===1&&migrated.learnedRecipes[0]==='antidote'; })()", "Crafting save migration did not sanitize stable IDs")
        ...
        check("Boolean(document.querySelector('.inventory-card') && document.querySelector('.equipment-slots') && document.querySelector('.pack-list') && document.querySelector('[data-action=\"start-expedition\"]'))", "Unified preparation screen is missing inventory, gear, pack, or expedition controls")
        ...
        for width, height in ((320, 480), (360, 640), (390, 844), (430, 932)):
            ...
            check("(() => { const prep=document.querySelector('.preparation-screen'); const button=document.querySelector('[data-action=\"start-expedition\"]'); prep.scrollTop=prep.scrollHeight; const box=button.getBoundingClientRect(); const viewport=document.querySelector('.game-viewport').getBoundingClientRect(); prep.scrollTop=0; return box.top>=viewport.top-1&&box.bottom<=viewport.bottom+1; })()", f"Begin Expedition is clipped at the end of preparation at {width}x{height}")
        ...
        devtools.click('[data-action="prepare-expedition"]')
        check("game.screen === 'preparation'", "Village preparation CTA did not open loadout")
