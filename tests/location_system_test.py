...
def run():
    ...
    try:
        ...
        check("EXPEDITION_TUNING.baseProvisionsPerDistance === 0.07 && !('maximumStartingProvisions' in EXPEDITION_TUNING)", "Provision tuning still uses the old cap or rate")
        ...
        check("Math.abs(EXPEDITION_TUNING.baseProvisionsPerDistance * game.expedition.provisionConsumptionMultiplier - 0.091) < 0.000001", "Kay's effective provision rate is invalid")
        ...
        check("new Set(Object.values(ENCOUNTER_DEFINITIONS).flatMap(encounter => Object.values(encounter.stages).flatMap(stage => stage.choices || []).flatMap(choice => choice.outcomes || []).filter(effect => effect.type === 'gainRandomUnsecuredItem').flatMap(effect => effect.itemIds))).size >= 15", "Encounter loot pools remain too repetitive")
