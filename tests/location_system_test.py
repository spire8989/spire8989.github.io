...
def run():
    ...
    try:
        ...
        check("(() => { const image=document.querySelector('.dialogue-portrait-image'); const fallback=document.querySelector('.portrait-fallback'); if (!fallback) return false; return image ? getComputedStyle(fallback).visibility==='hidden' : getComputedStyle(fallback).visibility==='visible'; })()", "Portrait initials were shown over a loaded portrait or the missing-image fallback disappeared")
        ...
        devtools.click('[data-action="show-location"]')
        check("Object.keys(MATERIAL_DEFINITIONS).length === 10 && Object.keys(RECIPE_DEFINITIONS).length === 11", "Crafting content definitions are incomplete")
        ...
        check("(() => { const recipesValid=Object.values(RECIPE_DEFINITIONS).every(recipe=>{ const ingredients=CraftingRules.normalizeRecipeIngredients(recipe).every(entry=>(entry.type==='item'?ITEM_DEFINITIONS[entry.id]:MATERIAL_DEFINITIONS[entry.id])&&Number.isInteger(entry.quantity)&&entry.quantity>0); const output=recipe.output.provisions>0||ITEM_DEFINITIONS[recipe.output.itemId]; return CRAFTING_PROVIDER_DEFINITIONS[recipe.craftingProvider]&&output&&RARITY_DEFINITIONS[recipe.rarity]&&ingredients; }); const entryValid=entry=>Number(entry.weight)>0&&({gold:true,material:Boolean(MATERIAL_DEFINITIONS[entry.materialId]),item:Boolean(ITEM_DEFINITIONS[entry.itemId]),recipe:Boolean(RECIPE_DEFINITIONS[entry.recipeId]),table:Boolean(LOOT_TABLE_DEFINITIONS[entry.tableId])})[entry.type]; return recipesValid&&Object.values(LOOT_TABLE_DEFINITIONS).every(table=>table.entries.length>0&&table.entries.every(entryValid))&&EXPEDITION_RETURN_REWARD_TIERS.every(tier=>tier.sources.every(source=>LOOT_TABLE_DEFINITIONS[source.tableId]&&source.rolls>0)); })()", "Recipe, loot-table, or return-tier references are invalid")
        check("game.player.saveVersion === 12 && game.player.learnedRecipes.includes('bandages') && game.player.materials.cloth === 3 && game.player.materials.raw_meat === 2 && !game.player.ownedItems.raw_meat && MaterialRules.collectionTotal(game.player.packedMaterials) === 10", "Fresh crafting state is invalid")
        check("(() => { const migrated=sanitizePlayerState({saveVersion:6,materials:{iron:2,unknown:9},ownedItems:{raw_meat:2},packedItems:['raw_meat'],learnedRecipes:['antidote','unknown'],injuries:{arthur:['deep_cut','deep_cut','unknown']}},SaveSystem.createDefaultPlayerState()); return migrated.saveVersion===12&&migrated.materials.iron===2&&migrated.materials.raw_meat===2&&!migrated.materials.unknown&&!migrated.ownedItems.raw_meat&&!migrated.packedItems.includes('raw_meat')&&migrated.packedMaterials.raw_meat===2&&migrated.learnedRecipes.length===1&&migrated.learnedRecipes[0]==='antidote'&&migrated.injuries.arthur.length===1&&migrated.injuries.arthur[0].injuryId==='deep_cut'&&Number.isFinite(migrated.injuries.arthur[0].remainingRecoveryDistance)&&migrated.campaignFlags.broceliande_intro_complete; })()", "Crafting save migration did not sanitize stable IDs, injuries, or move ingredients into the Material Bag")
        ...
        check("document.body.textContent.includes('22 / 45') && document.body.textContent.includes('32 / 45') && document.body.textContent.includes('Rest · ' + HEALING_TUNING.innRestGoldCost + 'g')", "Injured Inn state does not show current, restored, and cost values")
        ...
        devtools.evaluate("game.player.arthurHealth=45; game.player.companionStates.sir_kay.health=50; renderDestination()")
        ...
        check("(async () => { const path=AssetCatalog.imagePath('expedition_old_forest_road_bg'); if (!path) return true; const waitForActive=frame=>new Promise(resolve=>{let ticks=0; const poll=()=>frame.classList.contains('asset-image-active')||frame.classList.contains('asset-load-failed')||ticks++>100?resolve():setTimeout(poll,10); poll();}); const frame=document.querySelector('.travel-scene'); const image=document.querySelector('.travel-visual-asset'); if (!image) return false; await waitForActive(frame); const hidden=className => getComputedStyle(frame.querySelector(className)).display==='none'; const initialHidden=['.moon','.forest-far','.forest-near','.ground'].every(hidden); const initialTravelers=Boolean(frame.querySelector('.travelers')); const initialBanner=Boolean(frame.querySelector('.direction-banner')); const pausedBefore=game.expedition.travelState; game.expedition.travelState='paused'; renderExpedition(); const pausedFrame=document.querySelector('.travel-scene'); const pausedImage=document.querySelector('.travel-visual-asset'); await waitForActive(pausedFrame); const pausedFilter=getComputedStyle(pausedImage).filter; game.expedition.travelState=pausedBefore; renderExpedition(); return frame.classList.contains('asset-image-active') && initialHidden && initialTravelers && initialBanner && pausedFrame.classList.contains('asset-image-active') && pausedFilter!=='none'; })()", "Real travel artwork did not replace the moon/forest/ground fallback layers or retain paused treatment")
        check("(async () => { const definition=EXPEDITION_DEFINITIONS.old_forest_road; const original=definition.travelScenes; definition.travelScenes=[{minDistance:0,visualAssetId:'expedition_old_forest_road_bg'},{minDistance:2,visualAssetId:'expedition_old_forest_road_camp_bg'}]; const expedition=game.expedition; const originalState={status:expedition.status,travelState:expedition.travelState,direction:expedition.direction,distance:expedition.distance,maxDistanceReached:expedition.maxDistanceReached}; expedition.status='visual-test'; expedition.travelState='traveling'; expedition.direction='outbound'; expedition.distance=0; renderExpedition(); const wait=ms=>new Promise(resolve=>setTimeout(resolve,ms)); await wait(80); const first=document.querySelector('#travel-art')?.dataset.travelAssetId; expedition.distance=3; updateTravelHud(); const scene=document.querySelector('.travel-scene'); const overlap=scene?.classList.contains('asset-image-active') && document.querySelectorAll('#travel-art .travel-visual-asset').length>=2; await wait(820); const outbound=document.querySelector('#travel-art')?.dataset.travelAssetId; expedition.direction='returning'; expedition.distance=0; updateTravelHud(); await wait(820); const returning=document.querySelector('#travel-art')?.dataset.travelAssetId; const motion=getComputedStyle(document.querySelector('.travel-visual-asset')).animationDuration; definition.travelScenes=original; Object.assign(expedition,originalState); renderExpedition(); return first==='expedition_old_forest_road_bg' && overlap && outbound==='expedition_old_forest_road_camp_bg' && returning==='expedition_old_forest_road_bg' && motion!=='0s'; })()", "Distance-based travel scenes did not crossfade, reverse on return, or retain cinematic motion")
        check("(() => { const original=game.expedition.expeditionId; game.expedition.expeditionId='fountain_of_barenton'; renderExpedition(); const frame=document.querySelector('.travel-scene'); const visible=className => getComputedStyle(frame.querySelector(className)).display!=='none'; const result=!frame.querySelector('.travel-visual-asset') && visible('.moon') && visible('.forest-far') && visible('.forest-near') && visible('.ground') && Boolean(frame.querySelector('.direction-banner')); game.expedition.expeditionId=original; renderExpedition(); return result; })()", "Travel without an image did not retain the CSS fallback scene")
        check("(async () => { const path=AssetCatalog.imagePath('expedition_old_forest_road_bg'); if (!path) return true; const image=document.querySelector('.travel-visual-asset'); const frame=document.querySelector('.travel-scene'); const failed=new Promise(resolve => image.addEventListener('error',resolve,{once:true})); image.src='assets/images/expedition/__missing-travel-test__.webp'; await failed; const visible=className => getComputedStyle(frame.querySelector(className)).display!=='none'; const result=image.hidden && !frame.classList.contains('asset-image-active') && visible('.moon') && visible('.forest-far') && visible('.forest-near') && visible('.ground'); renderExpedition(); return result; })()", "Failed travel artwork did not restore the CSS fallback scene")
        ...
        check("!document.querySelector('.expedition-injury-summary') && !document.querySelector('.injury-panel') && !document.body.textContent.includes('Company injuries') && !document.body.textContent.includes('No persistent injuries')", "A healthy expedition still renders an empty injury section")
        ...
        devtools.click('.journey-log .run-details-summary')
        check("craftingRow(RECIPE_DEFINITIONS.repair_kit,'blacksmith').includes('Creates Repair Kit') && craftingRow(RECIPE_DEFINITIONS.roasted_meat,'campfire').includes('Creates 3 Provisions') && craftingRow(RECIPE_DEFINITIONS.strong_tonic,'apothecary').includes('Creates Strong Tonic') && RECIPE_DEFINITIONS.strong_tonic.output.quantity===2", "Crafting output labels or Strong Tonic quantity are invalid")
        ...
        devtools.evaluate("InjuryRules.applyToExpedition(game.expedition,'arthur','sprained_ankle',{source:'ui-test'}); renderExpedition()")
        check("(() => { const summary=document.querySelector('.expedition-injury-summary'); const entries=[...document.querySelectorAll('.expedition-injury-entry')]; return summary && entries.length===1 && entries[0].textContent.includes('Arthur') && entries[0].textContent.includes('Sprained Ankle') && entries[0].querySelector('.injury-tone-recoverable') && !summary.querySelector('p') && !summary.querySelector('.injury-panel'); })()", "Travel HUD did not show one active injury in a compact readable row")
        devtools.evaluate("InjuryRules.applyToExpedition(game.expedition,'sir_kay','bruised_ribs',{source:'ui-test'}); renderExpedition()")
        check("(() => { const summary=document.querySelector('.expedition-injury-summary'); const entries=[...document.querySelectorAll('.expedition-injury-entry')]; return summary && entries.length===2 && entries.some(entry=>entry.textContent.includes('Arthur')&&entry.textContent.includes('Sprained Ankle')&&entry.querySelector('.injury-tone-recoverable')) && entries.some(entry=>entry.textContent.includes('Sir Kay')&&entry.textContent.includes('Bruised Ribs')&&entry.querySelector('.injury-tone-recoverable')) && injurySemanticTone('infection')==='injury-tone-serious' && injurySemanticTone('poisoned')==='injury-tone-serious'; })()", "Travel HUD did not show a compact readable injury row for Arthur and the active companion")
        ...
        check("Boolean(document.querySelector('.expedition-injury-summary')) && document.querySelectorAll('.expedition-injury-entry').length===2", "Paused travel hid or lost the compact injury row")
        devtools.click('[data-action="make-camp"]')
        check("(async () => { const path=AssetCatalog.imagePath('expedition_old_forest_road_camp_bg'); if (!path) return true; const waitForActive=frame=>new Promise(resolve=>{let ticks=0; const poll=()=>frame.classList.contains('asset-image-active')||frame.classList.contains('asset-load-failed')||ticks++>100?resolve():setTimeout(poll,10); poll();}); const frame=document.querySelector('.camp-scene'); const image=document.querySelector('.camp-visual-asset'); if (!image) return false; await waitForActive(frame); const hidden=className => getComputedStyle(frame.querySelector(className)).display==='none'; return frame.classList.contains('asset-image-active') && hidden('.camp-moon') && hidden('.camp-fire') && hidden('.camp-silhouette') && Boolean(frame.querySelector('.direction-banner')); })()", "Real camp artwork did not replace the moon/fire/silhouette fallback layers or retain the direction banner")
        check("(() => { const original=game.expedition.expeditionId; game.expedition.expeditionId='fountain_of_barenton'; renderCamp(game.expedition); const frame=document.querySelector('.camp-scene'); const visible=className => getComputedStyle(frame.querySelector(className)).display!=='none'; const result=!frame.querySelector('.camp-visual-asset') && visible('.camp-moon') && visible('.camp-fire') && visible('.camp-silhouette'); game.expedition.expeditionId=original; renderCamp(game.expedition); return result; })()", "Camp without an image did not retain the CSS fallback scene")
        check("(async () => { const path=AssetCatalog.imagePath('expedition_old_forest_road_camp_bg'); if (!path) return true; const image=document.querySelector('.camp-visual-asset'); const frame=document.querySelector('.camp-scene'); const failed=new Promise(resolve => image.addEventListener('error',resolve,{once:true})); image.src='assets/images/expedition/__missing-camp-test__.webp'; await failed; const visible=className => getComputedStyle(frame.querySelector(className)).display!=='none'; const result=image.hidden && !frame.classList.contains('asset-image-active') && visible('.camp-moon') && visible('.camp-fire') && visible('.camp-silhouette'); renderCamp(game.expedition); return result; })()", "Failed camp artwork did not restore the CSS fallback scene")
        check("!document.querySelector('.expedition-action-bar')", "Sticky travel actions incorrectly appeared on the camp screen")
        check("(() => { const screen=document.querySelector('.camp-screen').getBoundingClientRect(); const panel=document.querySelector('.camp-panel').getBoundingClientRect(); const footer=document.querySelector('.camp-actions'); const footerBox=footer.getBoundingClientRect(); return document.querySelectorAll('.camp-panel .compact-resources .resource-card').length===7 && !document.querySelector('.expedition-injury-summary') && !document.querySelector('.injury-panel') && getComputedStyle(document.querySelector('.camp-tabs')).position==='sticky' && footer.parentElement.classList.contains('camp-screen') && getComputedStyle(footer).position==='relative' && Math.abs(footerBox.bottom-screen.bottom)<1 && Math.abs(footerBox.top-panel.bottom)<1; })()", "Camp status grid or the external Leave Camp footer is not structurally anchored to the viewport")
        ...
        for width, height in ((320, 480), (360, 640), (390, 844), (430, 932)):
            ...
            check("(() => { const row=document.querySelector('.expedition-injury-summary'); const viewport=document.querySelector('.game-viewport').getBoundingClientRect(); return row && row.getBoundingClientRect().left>=viewport.left-1 && row.getBoundingClientRect().right<=viewport.right+1 && row.scrollWidth<=row.clientWidth+1; })()", f"Compact injury row overflows the portrait viewport at {width}x{height}")
        check("document.querySelectorAll('.compact-resources .resource-card').length===7 && [...document.querySelectorAll('.compact-resources .resource-card')].every(card=>getComputedStyle(card).gridColumn==='auto') && document.querySelector('#loot-count')?.textContent.trim()==='0' && !document.querySelector('.unsecured-card') && !document.querySelector('.unsecured-breakdown')", "Expedition Status did not use seven consistent cells with a compact zero Unsecured Loot value")
        ...
        devtools.evaluate("game.expedition.selectedCompanion=null; game.expedition.injuries={arthur:[],sir_kay:[],llamrei:[]}; game.expedition.activeEncounter=null; game.expedition.nextEncounterAt=999999; renderExpedition()")
        ...
        devtools.evaluate("InjuryRules.applyToExpedition(game.expedition,'arthur','sprained_ankle',{source:'ui-test'}); InjuryRules.applyToExpedition(game.expedition,'sir_kay','bruised_ribs',{source:'ui-test'})")
        check("EncounterManager.force(game.expedition, 'fallen_tree')", "Existing encounter could not be triggered")
        check(r'''(async () => { const originalExpedition=game.expedition; const originalScreen=game.screen; const originalTravelState=game.travelVisualState; const definition=EXPEDITION_DEFINITIONS.old_forest_road; const originalScenes=definition.travelScenes; const encounterDefinition=ENCOUNTER_DEFINITIONS.fallen_tree; const originalEncounterVisual=encounterDefinition.visualAssetId; const originalImagePath=AssetCatalog.imagePath; const panoramaId='expedition_old_forest_road_wide_bg'; const otherId='expedition_old_forest_road_bg'; const panoramaSrc='data:image/svg+xml;charset=utf-8,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="3000" height="1000"><rect width="3000" height="1000" fill="#3b5945"/></svg>'); const waitForActive=frame=>new Promise(resolve=>{let ticks=0; const poll=()=>frame?.classList.contains('asset-image-active')||frame?.classList.contains('asset-load-failed')||ticks++>100?resolve():setTimeout(poll,10); poll();}); const nextFrame=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))); let result=false; try { definition.travelScenes=[{minDistance:0,visualAssetId:panoramaId},{minDistance:10,visualAssetId:otherId}]; AssetCatalog.imagePath=id=>id===panoramaId?panoramaSrc:originalImagePath.call(AssetCatalog,id); const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:[],provisions:10,random:()=>0}); expedition.status='active'; expedition.travelState='traveling'; expedition.direction='outbound'; expedition.distance=0; expedition.maxDistanceReached=0; game.expedition=expedition; game.screen='expedition'; encounterDefinition.visualAssetId=null; renderExpedition(); let frame=document.querySelector('.travel-scene'); await waitForActive(frame); let image=document.querySelector('#travel-art .travel-visual-asset'); let animation=travelImageAnimation(image); animation.currentTime=7000; const savedPosition=Number(animation.currentTime); if (!Number.isFinite(savedPosition)) return false; if (!EncounterManager.force(expedition,'fallen_tree')) return false; renderExpedition(); const savedDuringEncounter=game.travelVisualState?.assetId===panoramaId && Number.isFinite(game.travelVisualState?.currentTime); const encounterUsesTravelArtwork=document.querySelector('#travel-art .travel-visual-asset')?.dataset.travelAssetId===panoramaId; expedition.activeEncounter.phase='result'; expedition.activeEncounter.resultText='The path is clear.'; continueJourney(); frame=document.querySelector('.travel-scene'); await waitForActive(frame); await nextFrame(); image=document.querySelector('#travel-art .travel-visual-asset'); expedition.travelState='paused'; updateTravelHud(); animation=travelImageAnimation(image); const returnedPosition=Number(animation?.currentTime); const restoredNearPosition=image?.dataset.travelAssetId===panoramaId && Math.abs(returnedPosition-savedPosition)<250; encounterDefinition.visualAssetId=otherId; expedition.travelState='traveling'; if (!EncounterManager.force(expedition,'fallen_tree')) return false; renderExpedition(); const encounterArtworkWorks=document.querySelector('#travel-art .travel-visual-asset')?.dataset.travelAssetId===otherId; expedition.activeEncounter.phase='result'; expedition.activeEncounter.resultText='The path is clear.'; continueJourney(); renderExpedition(); await waitForActive(document.querySelector('.travel-scene')); expedition.distance=0; expedition.travelState='traveling'; renderExpedition(); await waitForActive(document.querySelector('.travel-scene')); image=document.querySelector('#travel-art .travel-visual-asset'); animation=travelImageAnimation(image); animation.currentTime=6500; game.travelVisualState=captureTravelVisualState(expedition); expedition.distance=10; updateTravelHud(); const changedAssetClearsSavedState=game.travelVisualState===null; result=savedDuringEncounter && encounterUsesTravelArtwork && restoredNearPosition && encounterArtworkWorks && changedAssetClearsSavedState; } finally { AssetCatalog.imagePath=originalImagePath; definition.travelScenes=originalScenes; encounterDefinition.visualAssetId=originalEncounterVisual; game.travelVisualState=originalTravelState; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); } return result; })()''', "Travel encounter interruption did not preserve the same panorama position, defer encounter artwork correctly, or clear state for a changed scene")
        ...
        check("(() => { const panel=document.querySelector('.encounter-panel'); const resources=panel?.querySelector('.compact-resources'); return !document.querySelector('.expedition-injury-summary') && !document.querySelector('.injury-panel') && resources?.nextElementSibling?.classList.contains('encounter-heading'); })()", "Active encounter did not prioritize its heading by hiding the compact injury row")
        ...
        check("Object.keys(ENCOUNTER_DEFINITIONS).length === 71", "Unexpected encounter pool size after content expansion")
        check("COMBAT_DEFINITIONS.bandit_ambush.enemyIds.length===2 && COMBAT_ENEMY_DEFINITIONS.bandit.maxHp < COMBAT_ENEMY_DEFINITIONS.bandit_leader.maxHp && COMBAT_DEFINITIONS.bandit_leader.enemyIds.length===1", "Bandit ambush and leader combat definitions are incomplete")
        check("LOOT_TABLE_DEFINITIONS.bandit_ambush_loot && LOOT_TABLE_DEFINITIONS.bandit_leader_loot && LOOT_TABLE_DEFINITIONS.bandit_leader_loot.entries.some(entry=>entry.itemId==='blackthorn_badge')", "Bandit loot tables did not provide modest regular and leader rewards")
        check("ENCOUNTER_DEFINITIONS.fountain_barenton.minimumDistance===90 && !Number.isFinite(ENCOUNTER_DEFINITIONS.fountain_barenton.maximumDistance) && ENCOUNTER_DEFINITIONS.bandit_leader.requirements.some(requirement=>requirement.type==='runFlag'&&requirement.flag==='banditLeaderEligible') && ENCOUNTER_DEFINITIONS.bandit_leader.requirements.some(requirement=>requirement.type==='notRunFlag'&&requirement.flag==='banditLeaderDefeated')", "Fountain distance gating or bandit leader run requirements are invalid")
        check("(() => { const player=SaveSystem.createDefaultPlayerState(); const expedition=ExpeditionRules.createExpedition(player,{companions:[],provisions:10,random:()=>0}); expedition.distance=30; expedition.runFlags={}; const before=EncounterManager.eligibleDefinitions(expedition,player).some(entry=>entry.id==='bandit_leader'); expedition.runFlags.banditLeaderEligible=true; const after=EncounterManager.eligibleDefinitions(expedition,player).some(entry=>entry.id==='bandit_leader'); expedition.runFlags.banditLeaderDefeated=true; const defeated=EncounterManager.eligibleDefinitions(expedition,player).some(entry=>entry.id==='bandit_leader'); return !before&&after&&!defeated; })()", "Bandit leader eligibility did not use transient run flags")
        check("EXPEDITION_TUNING.outboundTravelSpeed === 1 && EXPEDITION_TUNING.encounterMinimumDistance === 7 && EXPEDITION_TUNING.encounterMaximumDistance === 10 && EXPEDITION_TUNING.postEncounterSafeDistance === 4.5", "Travel scale and encounter spacing are not on the rebalance target")
        ...
        devtools.evaluate("game.expedition.injuries={arthur:[],sir_kay:[],llamrei:[]}")

        check("PLAYER_CHARACTER_DEFINITION.combat.maxHp === 45 && ITEM_DEFINITIONS.arthur_sword.effects.combatDamage.minimum === 8 && ITEM_DEFINITIONS.quilted_hauberk.effects.combatDefense === 4 && ITEM_DEFINITIONS.reinforced_mail.effects.combatDefense === 6", "Arthur equipment or health stats are invalid")
        ...
        check("(() => { const sample={...game.expedition,selectedCompanion:null,health:33,companionCombatHp:{}}; const combat=CombatSystem.create(sample,'wild_boar',{random:()=>0}); const arthur=combat.allies[0]; arthur.gauge=100; CombatSystem.update(combat,sample,0); CombatSystem.chooseAction(combat,sample,'defend'); combat.enemies[0].gauge=100; CombatSystem.update(combat,sample,0); const persisted=sample.health===31 && arthur.defending; arthur.gauge=100; CombatSystem.update(combat,sample,0); return persisted && !arthur.defending; })()", "Defend mitigation, duration, or Arthur expedition health persistence is invalid")
        ...
        check("game.expedition.combat?.allies.length===1 && game.expedition.combat.allies[0].id==='arthur' && document.querySelector('.combat-screen') && game.expedition.combat.status==='awaitingAction' && !document.querySelector('.expedition-action-bar') && !document.querySelector('.expedition-injury-summary')", "Solo Wild Boar Fight included Kay, failed to render its ready state, or showed travel actions")
        ...
        check(r'''(async () => { const originalExpedition=game.expedition; const originalScreen=game.screen; const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:[],provisions:5,random:()=>0}); expedition.status='visual-test'; expedition.travelState='traveling'; expedition.direction='outbound'; game.expedition=expedition; game.screen='expedition'; renderExpedition(); const image=document.querySelector('#travel-art .travel-visual-asset'); if (!image) { game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return false; } const loaded=new Promise(resolve=>image.addEventListener('load',resolve,{once:true})); image.src='data:image/svg+xml;charset=utf-8,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="3000" height="1000"><rect width="3000" height="1000" fill="#3b5945"/></svg>'); await loaded; await new Promise(resolve=>requestAnimationFrame(resolve)); const frame=document.querySelector('.travel-scene'); const range=Number.parseFloat(image.style.getPropertyValue('--travel-pan-range')); const panoramic=image.classList.contains('is-panorama') && image.dataset.travelAspect==='panorama' && range > frame.clientWidth * 0.2; const outboundDirection=getComputedStyle(image).animationDirection==='alternate'; const marker=document.querySelector('.travelers'); const markerLeft=marker.getBoundingClientRect().left; expedition.travelState='paused'; updateTravelHud(); const animation=travelImageAnimation(image); const before=Number(animation?.currentTime); await new Promise(resolve=>setTimeout(resolve,140)); const after=Number(animation?.currentTime); const markerStillFixed=Math.abs(marker.getBoundingClientRect().left-markerLeft)<1; expedition.paceId='cautious'; expedition.travelState='traveling'; updateTravelHud(); const cautiousDuration=getComputedStyle(image).animationDuration; expedition.direction='returning'; updateTravelHud(); const returnDirection=getComputedStyle(image).animationDirection==='alternate-reverse'; const result=panoramic && outboundDirection && Math.abs(after-before)<2 && markerStillFixed && cautiousDuration==='20s' && returnDirection; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); return result; })()''', "3:1 travel panoramas did not fill and pan across the frame, pause in place, respect pace, or reverse on return")
        check(r'''(async () => { const originalExpedition=game.expedition; const originalScreen=game.screen; const originalTravelState=game.travelVisualState; const definition=EXPEDITION_DEFINITIONS.old_forest_road; const originalScenes=definition.travelScenes; const encounterDefinition=ENCOUNTER_DEFINITIONS.fallen_tree; const originalEncounterVisual=encounterDefinition.visualAssetId; const originalImagePath=AssetCatalog.imagePath; const panoramaId='expedition_old_forest_road_wide_bg'; const panoramaSrc='data:image/svg+xml;charset=utf-8,'+encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="3000" height="1000"><rect width="3000" height="1000" fill="#3b5945"/></svg>'); const waitForActive=frame=>new Promise(resolve=>{let ticks=0; const poll=()=>frame?.classList.contains('asset-image-active')||frame?.classList.contains('asset-load-failed')||ticks++>100?resolve():setTimeout(poll,10); poll();}); const nextFrame=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))); let result=false; try { definition.travelScenes=[{minDistance:0,visualAssetId:panoramaId}]; encounterDefinition.visualAssetId=null; AssetCatalog.imagePath=id=>id===panoramaId?panoramaSrc:originalImagePath.call(AssetCatalog,id); const expedition=ExpeditionRules.createExpedition(game.player,{expeditionId:'old_forest_road',companions:[],provisions:10,random:()=>0}); expedition.status='active'; expedition.travelState='traveling'; expedition.direction='outbound'; game.expedition=expedition; game.screen='expedition'; renderExpedition(); await waitForActive(document.querySelector('.travel-scene')); let image=document.querySelector('#travel-art .travel-visual-asset'); let animation=travelImageAnimation(image); animation.currentTime=5200; const expected=Number(animation.currentTime); EncounterManager.force(expedition,'fallen_tree'); renderExpedition(); await waitForActive(document.querySelector('.travel-scene')); await nextFrame(); image=document.querySelector('#travel-art .travel-visual-asset'); animation=travelImageAnimation(image); const during=Number(animation?.currentTime); await new Promise(resolve=>setTimeout(resolve,120)); const frozen=Number(animation?.currentTime); expedition.activeEncounter.phase='result'; expedition.activeEncounter.resultText='The path is clear.'; continueJourney(); await waitForActive(document.querySelector('.travel-scene')); await nextFrame(); expedition.travelState='paused'; updateTravelHud(); image=document.querySelector('#travel-art .travel-visual-asset'); const resumed=Number(travelImageAnimation(image)?.currentTime); result=image?.dataset.travelAssetId===panoramaId && Math.abs(during-expected)<250 && Math.abs(frozen-during)<2 && Math.abs(resumed-expected)<250; } finally { AssetCatalog.imagePath=originalImagePath; definition.travelScenes=originalScenes; encounterDefinition.visualAssetId=originalEncounterVisual; game.travelVisualState=originalTravelState; game.expedition=originalExpedition; game.screen=originalScreen; renderScreen(); } return result; })()''', "Encounter without dedicated artwork did not freeze and resume the travel panorama at its interruption position")
