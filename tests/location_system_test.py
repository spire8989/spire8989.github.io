...
def run():
    ...
    try:
        ...
        devtools.click('[data-action="show-location"]')
        check("Object.keys(MATERIAL_DEFINITIONS).length === 10 && Object.keys(RECIPE_DEFINITIONS).length === 5", "Crafting content definitions are incomplete")
        check("(() => { const recipesValid=Object.values(RECIPE_DEFINITIONS).every(recipe=>CRAFTING_PROVIDER_DEFINITIONS[recipe.craftingProvider]&&ITEM_DEFINITIONS[recipe.output.itemId]&&RARITY_DEFINITIONS[recipe.rarity]&&Object.entries(recipe.ingredients).every(([id,quantity])=>MATERIAL_DEFINITIONS[id]&&Number.isInteger(quantity)&&quantity>0)); const entryValid=entry=>Number(entry.weight)>0&&({gold:true,material:Boolean(MATERIAL_DEFINITIONS[entry.materialId]),item:Boolean(ITEM_DEFINITIONS[entry.itemId]),recipe:Boolean(RECIPE_DEFINITIONS[entry.recipeId]),table:Boolean(LOOT_TABLE_DEFINITIONS[entry.tableId])})[entry.type]; return recipesValid&&Object.values(LOOT_TABLE_DEFINITIONS).every(table=>table.entries.length>0&&table.entries.every(entryValid))&&EXPEDITION_RETURN_REWARD_TIERS.every(tier=>tier.sources.every(source=>LOOT_TABLE_DEFINITIONS[source.tableId]&&source.rolls>0)); })()", "Recipe, loot-table, or return-tier references are invalid")
        ...
        check("Object.keys(ENCOUNTER_DEFINITIONS).length === 33", "Unexpected encounter pool size after content expansion")
