...
def run():
    ...
    try:
        ...
        check("game.player.saveVersion === 9 && game.player.learnedRecipes.includes('bandages') && game.player.materials.cloth === 3 && game.player.materials.raw_meat === 2 && !game.player.ownedItems.raw_meat && MaterialRules.collectionTotal(game.player.packedMaterials) === 10", "Fresh crafting state is invalid")
        check("(() => { const migrated=sanitizePlayerState({saveVersion:6,materials:{iron:2,unknown:9},ownedItems:{raw_meat:2},packedItems:['raw_meat'],learnedRecipes:['antidote','unknown']},SaveSystem.createDefaultPlayerState()); return migrated.saveVersion===9&&migrated.materials.iron===2&&migrated.materials.raw_meat===2&&!migrated.materials.unknown&&!migrated.ownedItems.raw_meat&&!migrated.packedItems.includes('raw_meat')&&migrated.packedMaterials.raw_meat===2&&migrated.learnedRecipes.length===1&&migrated.learnedRecipes[0]==='antidote'&&migrated.campaignFlags.broceliande_intro_complete; })()", "Crafting save migration did not sanitize stable IDs or move ingredients into the Material Bag")
