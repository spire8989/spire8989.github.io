...
Visual replay is available from the `?sim=1` developer panel. Select a single expedition and choose **Watch Replay**, or select a campaign and choose **Watch Campaign Replay**. The single-run viewer creates a sandboxed player/expedition from `run.replay`; the campaign viewer creates a sandboxed campaign player, applies ordered town actions, and delegates each expedition to the same `ReplayController`. `ReplayData.normalize(run)` and `CampaignReplayData.normalize(campaign)` provide stable internal shapes; both controllers consume recorded decisions in order, call production rules, and render through the normal game UI. Compact Campaign JSON is intentionally not a replay source because v2 omits decision/action detail.

The single-run viewer supports pace/ration changes, fixed or provision-based turnarounds, brief rest, camp, campfire cooking, camp events, encounter choices, combat actions/abilities/items/targets, and leave-camp. Campaign playback additionally shows town entry, Inn rest/cooking, purchases, sales, crafting, injury treatment, equipment/equip replacement, companions, pack/Material Bag preparation, return summaries, and campaign transitions. Campaign controls are stable after mounting, and long campaign seeks/skips advance in bounded yielding chunks so Pause and Exit remain usable. Both viewers pause with structured desync errors if a recorded action is missing, unavailable, or aimed at a different state. Replay mutations never call `SaveSystem.save`; exiting restores the prior game object references and screen.
