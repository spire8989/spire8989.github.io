...
- Aggressive campaign healing triggers below 50% Arthur HP; below 25%, it may automate one additional shared production Inn rest if the first leaves Arthur below 50% and another is affordable. Aggressive simulation combat normally attacks; only deterministic predicted-lethal damage before Arthur's next action may select Defend or Flee, and that exception must remain visible in telemetry.
- Production enemy actions use injected combat RNG to select among living party members with centralized weights (currently Arthur 65%, active companions collectively 35%). Intercede may redirect an Arthur-selected attack; telemetry preserves both selected and final targets. Never spread damage only in simulation.
...
- Arthur's base maximum health is data-driven (currently 45), as is each companion maximum (Sir Kay is currently 50). `player.arthurHealth` and `player.companionStates` persist through save version 6, settlement, town entry, and the next expedition. One flat-cost Inn rest heals Arthur and the selected companion through the same `HealingRules` party operation used by campaign simulation; never add a separate simulator healing formula or per-character rest charge.
