...
Automated campaign preparation uses known Bandage recipes before falling back to the real General Goods offer after healing and provision planning. Aggressive, Cautious, and Random strategies choose deterministic targets, craft when materials allow, spend gold against finite stock for any shortfall, preserve existing utility pack items, and report crafting, purchases, packing, consumption, returns, and healing in campaign telemetry. See `CAMPAIGN_SIMULATION.md` for the policy and export details.
