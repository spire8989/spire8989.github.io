...
item, encounter, reward, and expedition overrides. Combat SFX resolve as
authored ability/enemy-action or expedition overrides, then global combat
defaults, then semantic fallbacks, then silence; timing remains driven by the
combat presentation impact rather than simulation resolution. `js/audio.js`
owns gesture unlocking, local mute/volume preferences, synth playback, and
semantic SFX hooks; missing synth IDs remain silent. The small header
